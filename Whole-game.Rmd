...
```{r, local_opts = list(tidyverse.quiet = TRUE)}
library(tidyverse)
library(usethis)
```
...
```{r create-package, include = debug, eval = create, echo = debug}
withr::with_options(
  ## don't pick up any usethis options set by the rendering user
...
```{r init-fbind, echo = FALSE, eval = create}
withr::with_envvar(
  c("TESTTHAT" = "true"),
  use_r("fbind")
)
...
```{r init-freq-out, echo = FALSE, eval = create}
withr::with_envvar(
  c("TESTTHAT" = "true"),
  use_r("freq_out")
)
