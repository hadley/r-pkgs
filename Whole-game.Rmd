...
proj_sitrep()
loadedNamespaces()
"foofactors" %in% loadedNamespaces()
pkgload::load_all()
knitr::knit_exit()
```
...
Before we tackle the warnings about the license, let's work on the boilerplate content in `DESCRIPTION`. The `DESCRIPTION` file provides metadata about your package and is covered fully in chapter \@ref(description).
...
```{r restore-wd, include = debug, eval = create}
# being kind someone working interactively and locally
if (is.null(getOption("knitr.in.progress"))) {
  setwd(owd)
  proj_set()
