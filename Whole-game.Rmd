...
```{r strsplit1-fodder, echo = FALSE, comment = NA}
strsplit1_fodder <- glue('
  strsplit1 <- function(x, split) {
    strsplit(x, split = split)[[1]]
  }',
  .open = "<<", .close = ">>"
)
cat(strsplit1_fodder)
```
...
```{r init-strsplit1}
#| eval = create, chunk_envvar = c(rlang_interactive = "FALSE")
use_r("strsplit1")
```

```{r write-strsplit1, include = FALSE, eval = create}
writeLines(strsplit1_fodder, path("R", "strsplit1.R"))
```
...
```{r description-fodder}
#| asis = TRUE, echo = FALSE, comment = NA, eval = create
DESCRIPTION_fodder <- glue('
  Package: regexcite
...
```{r strsplit1-roxygen-header, echo = FALSE, comment = NA, eval = create}
strsplit1_roxygen_header <- glue("
  #' Split a string
  #'
  ...
  #' x <- \"alfa,bravo,charlie,delta\"
  #' strsplit(x, split = \",\")")
strsplit1_safe <- readLines(path("R", "strsplit1.R"))
writeLines(
  c(strsplit1_roxygen_header, strsplit1_safe),
  path("R", "strsplit1.R")
)
cat(strsplit1_roxygen_header, sep = "\n")
```
...
```{r test-strsplit1-fodder, echo = FALSE, comment = NA, eval = create}
test_fodder <- glue('
...
```{r test}
#| eval = create, message = FALSE,
#| R.options = list(testthat.default_reporter = testthat::ProgressReporter$new(update_interval = Inf))
test()
```
...
```{r str-split-one-fodder, echo = FALSE, comment = NA}
str_split_one_fodder <- glue("
...
```{r str-split-one-write, echo = FALSE, comment = NA, eval = create}
str_split_one_roxygen_header <- glue("
  #' Split a string
  #'
...
writeLines(
  c(str_split_one_roxygen_header, str_split_one_fodder),
  path("R", "str_split_one.R")
)
```
...
```{r asis = TRUE, echo = FALSE, comment = NA, eval = create}
cat(readLines(path("R", "str_split_one.R")), sep = "\n")
```
...
```{r test-str-split-one-write, echo = FALSE, comment = NA, eval = create}
str_split_one_tests <- glue('
...
writeLines(
  str_split_one_tests,
  path("tests", "testthat", "test-str_split_one.R")
)
cat(readLines(path("tests", "testthat", "test-str_split_one.R")), sep = "\n")
```
