...
The `create_package()` function initializes a new package in a directory on your computer and creates the directory, if necessary. The new package will comply with all devtools conventions and will immediately pass `R CMD check`. Ideally, your package will always be in this happy state and it is something you should verify often.
...
Where do you put this function definition? Save it in a `.R` file, in the `R/` subdirectory of your package. A reasonable default is to make a new `.R` file for each function in your package and name the file after the function. This mean the above definition of `fbind()` belongs in the file `R/fbind.R`.

The usethis package has a helper `use_r()` to create and/or open a script below `R/`. It really shines in a mature package, when when navigating between `.R` files and their associated tests. But, even here, it's useful to keep yourself from getting too carried away while working in `Untitled4`.

```{r init-fbind-fake, eval = FALSE}
use_r("fbind")
```

```{r init-fbind, echo = FALSE, eval = create, chunk_envvar = c("TESTTHAT" = "true")}
use_r("fbind")
```
...
How do we test drive `fbind()`? If this were a regular R script, we might use Rstudio or Emacs/ESS to send the function definition to the R Console and define `fbind()` in the global workspace. Or perhaps `source("R/fbind.R")`. For package development, however, devtools offers a more robust approach.
...
This simulates the process of building, installing, and attaching the foofactors package. In the long run, as your package accumulates more functions, some exported, some not, some of which call each other, `load_all()` gives you the most accurate sense of how the package is developing.

Now we can call `fbind(a, b)` to see how it works.
...
Let's start adding package-level documentation. Your package as a whole can have its own `.Rd` file. You can see an example by entering `package?tibble` in R Console. Read more about the uses of package-level documentation [later in this book](#man-packages).
...
This creates a peculiar dummy file `R/foofactors-package.R` with a roxygen header, that is just documenting `NULL`. The purpose of this is simply to trigger the generation of a package-level `.Rd` file. By default, this package-level documentation will be automatically generated from metadata in `DESCRIPTION`. To achieve this, we re-`document()` the package.
...
paths <- unlist(status(repo))
add(repo, path = paths)
commit(repo, message = "Add package-level doc, re-document()")
```
...
A piece of package-level documentation that's probably familiar to you is the vignette. This is a great place to put a fully developed example that calls multiple functions from your package to do something useful and realistic. See the [vignettes chapter](#vignettes) for more.

Use `devtools::use_vignette()` to initialize a new vignette:

```{r use-vignette, eval = create, chunk_envvar = c("TESTTHAT" = "true")}
...
```{r copy-vignette, asis = TRUE, echo = FALSE, comment = NA, include = debug, eval = create}
vignette_path <- path("vignettes", "hello-foofactors.Rmd")
owd
file_copy(
  path(owd, "fixtures", "foofactors-vignette.Rmd"),
  vignette_path,
  overwrite = TRUE
)
```
