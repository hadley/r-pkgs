...
destdir <- switch(
  dest,
  tmp_user = fs::path_home("tmp", "foofactors"),
  tmp_session = fs::path_temp("foofactors"),
  NULL
)
if (!is.null(destdir)) {
...
This chapter runs through the development of a small toy package, from beginning to end. It's meant to give you a sense of the Big Picture, before we delve into the details for each of the key components of an R package. To keep the pace brisk, we make heavy use of modern conveniences in the devtools package and the RStudio IDE. In later chapters, we are more explicit about what those helpers are doing for us.

*Work in progress! Adapting from material originally developed as part of STAT 545 and as a possible vignette for devtools.*

```{r params-workaround}
## detritus from this being a standalone document with parametrized yaml
## I just need this to render at all, before I can truly integrate it
params <- list(debug = FALSE, github = FALSE)
```
...
In the past, I link out to a page that gives instructions for:

  * Windows: Rtools
  * Mac: Xcode CLT
  * Linux: ?I wave my hands here ... is `r-devel` or `r-base-dev` needed?

Maybe this can be skipped if the immediate goal is to build a pure R package? Or an internal link to similar content within the book?
...
Make a deliberate choice about where to create this package on your computer. It should probably be somewhere within your home directory, alongside your other R projects. It should not be in, for example, an R package library, which holds packages that have already been built and installed. The conversion of the source package we are creating here into an installed package is part of what devtools facilitates. Don't try to do devtools' job for it.

Here we create a new package in a new directory with `create_package()`. We'll have to work in a temp directory, but if you're following along, you should substitute the correct path for **your** system here.

```{r create-package-fake, eval = FALSE}
create_package("path/to/foofactors")
```

```{r create-package, eval = create}
create_package(destdir, open = FALSE, rstudio = TRUE)
```

```{r activate-package, echo = FALSE, eval = create}
usethis::proj_set(destdir)
```

```{r set-root-dir, include = params$debug, eval = FALSE}
...
```{r check-wd, include = params$debug, eval = FALSE}
## make sure new, correct wd is in effect
## needs to be in a separate chunk, because knitr
getwd()
```

If you use RStudio, you should find yourself in a new instance of RStudio, opened into your new foofactors package (and Project). If you somehow need to do this manually, navigate to the directory and double click on `foofactors.Rproj`. RStudio has special handling for packages and you should now see a *Build* tab in the same pane as *Environment* and *History*.

*good place for a screenshot*

What's in this new directory that is also an R package and, possibly, an RStudio Project? Here's a listing (locally, you can consult your file browser):

```{r init-show-files, eval = create}
proj_get() %>% 
  dir_info(all = TRUE) %>% 
  select(path, type)
```
...
```{r init-list-files, include = FALSE, eval = create}
listing_1 <- dir_ls(proj_get(), all = TRUE)
```
...
*In an interactive session, you might be asked if you want to commit some files here.*

```{r git2r-begin, include = params$debug, eval = create}
library(git2r)
discover_repository(destdir)
```
...
```{r post-git-init-list-files, eval = create}
listing_2 <- dir_ls(proj_get(), all = TRUE)
setdiff(listing_2, listing_1)
```

If you're using RStudio, it probably requested permission to relaunch itself in this Project. You can do so manually by quitting and relaunching by double clicking on `foofactors.Rproj`. Now, in addition to package development support, you have access to a basic Git client in the *Git* tab of the *Environment/History/Build* pane. Click on History (the clock icon) and, if you consented, you might see an initial commit made via `use_git()`:

```{r inspect-first-commit, echo = FALSE, eval = FALSE}
## now that use_git() does not necessarily commit, we can't assume there is
## a history
...
writeLines(fbind_fodder, proj_path("R", "fbind.R"))
...
```{r load-all-fake, eval = FALSE}
load_all()
```

```{r load-all, include = FALSE, eval = create}
load_all(proj_get())
```
...
```{r fbind-init, echo = params$debug, comment = NA, include = FALSE, eval = FALSE}
...
writeLines(DESCRIPTION_fodder, proj_path("DESCRIPTION"))
cat(DESCRIPTION_fodder, sep = "\n")
```

```{r commit-description, echo = params$debug, comment = NA, eval = FALSE}
...
For foofactors, we use the MIT license. This requires specification in `DESCRIPTION` and a file called `LICENSE`. We do this now via the helper function `devtools::use_mit_license()`.
...
```{r license-fodder, asis = TRUE, echo = FALSE, comment = NA, eval = FALSE}
...
```{r commit-license, echo = params$debug, comment = NA, eval = FALSE}
...
fbind_safe <- readLines(proj_path("R", "fbind.R"))
writeLines(c(fbind_roxygen_header, paste(fbind_safe, collapse = "\n")),
           proj_path("R", "fbind.R"))
cat(fbind_roxygen_header, sep = "\n")
```

```{r commit-fbind-roxygen-header, echo = params$debug, comment = NA, eval = FALSE}
add(repo, path = file.path("R", "fbind.R"))
...
cat(readLines(proj_path("NAMESPACE")), sep = "\n")
```
...
```{r commit-namespace, echo = params$debug, comment = NA, eval = FALSE}
...
```{r commit-testthat-init, echo = params$debug, comment = NA, eval = FALSE}
...
test_path <- proj_path("tests", "testthat", "test_fbind.R")
...
```{r commit-fbind-test, echo = params$debug, comment = NA, eval = FALSE}
paths <- unlist(status(repo))
add(repo, path = paths)
...
```{r commit-forcats-imports, echo = params$debug, comment = NA, eval = FALSE}
paths <- unlist(status(repo))
add(repo, path = paths)
...
Now we add a new function to foofactors that does, indeed, use a function from forcats. Imagine we want a frequency table for a factor, as a regular data frame with nice variable names, versus as an object of class `table` or something with odd names. Preface your calls to forcats functions with `forcats::`.

Create a new file `R/freq_out.R` with this function definition:

```{r freq-out-fodder, asis = TRUE, echo = FALSE, comment = NA, eval = create}
freq_out_fodder <- c(
...
writeLines(freq_out_fodder, proj_path("R", "freq_out.R"))
cat(freq_out_fodder, sep = "\n")
```

```{r commit-freq-out, echo = params$debug, comment = NA, eval = FALSE}
paths <- unlist(status(repo))
add(repo, path = paths)
...
```{r commit-freq-out-rd, echo = params$debug, comment = NA, eval = FALSE}
paths <- unlist(status(repo))
add(repo, path = paths)
...
This creates a peculiar dummy file `R/foofactors-package.R` with a roxygen header, that is just documenting `NULL`. The purpose of this is simply to trigger the generation of a package-level `.Rd` file.
...
```{r pkg-doc-fodder, asis = TRUE, echo = FALSE, comment = NA, eval = FALSE, include = FALSE}
...
```{r commit-package-doc, echo = params$debug, comment = NA, eval = FALSE}
paths <- unlist(status(repo))
add(repo, path = paths)
...
```{r commit-package-Rd, echo = params$debug, comment = NA, eval = FALSE}
paths <- unlist(status(repo))
add(repo, path = paths)
commit(repo, message = "Run document()")
```
...
```{r commit-boilerplate-vignette, echo = params$debug, comment = NA, eval = FALSE}
paths <- unlist(status(repo))
add(repo, path = paths)
...
```{r copy-vignette, asis = TRUE, echo = FALSE, comment = NA, include = params$debug, eval = FALSE}
vignette_path <- proj_path("vignettes", "hello-foofactors.Rmd")
# FIXME: bring here or download from web?
# file.copy(file.path(owd, "foofactors-vignette.Rmd"),
#           vignette_path, overwrite = TRUE)
#cat(readLines(vignette_path), sep = "\n")
```

```{r commit-vignette, echo = params$debug, comment = NA, eval = FALSE}
paths <- unlist(status(repo))
add(repo, path = paths)
...
```{r use-github, echo = FALSE, eval = params$github, eval = FALSE}
#use_github(protocol = "https")
use_github(protocol = "https", private = TRUE)
```
...
```{r commit-readme-rmd, echo = params$debug, comment = NA, eval = FALSE}
paths <- unlist(status(repo))
add(repo, path = paths)
...
```{r copy-readme-rmd, include = params$debug, eval = FALSE}
# FIXME
# file.copy(file.path(owd, "foofactors-README.Rmd"),
#           "README.Rmd", overwrite = TRUE)
```

```{r cat-readme-rmd, asis = TRUE, echo = FALSE, comment = NA, include = FALSE, eval = create}
# FIXME
...
```{r render-readme-rmd, include = params$debug, eval = create}
# FIXME
...
```{r commit-rendered-readme, echo = params$debug, comment = NA, eval = FALSE}
paths <- unlist(status(repo))
add(repo, path = paths)
commit(repo, message = "Write README.Rmd and render")
```

```{r final-push, eval = params$github, include = params$debug, eval = FALSE}
...
```{r teardown, include = params$debug, eval = create}
