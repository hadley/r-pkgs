...
TIP: In RStudio's file browser, go to *More>  Show Hidden Files* to toggle the visibility of hidden files (a.k.a. ["dotfiles"](https://en.wikipedia.org/wiki/Hidden_file_and_hidden_directory#Unix_and_Unix-like_environments)). A select few are visible all the time, but sometimes you want to see them all.
...
TIP: The use of Git or another version control system is optional, but a recommended practice in the long-term. We explain its importance in FUTURE LINK (FIXME: link to right place when exists).
...
TIP: RStudio can initialize a Git repository, in any Project, even if it's not an R package: *Tools > Version Control > Project Setup*. Then choose *Version control system: Git* and *initialize a new git repository for this project*.
...
TIP: RStudio exposes `load_all()` via *Build > Load All*, in the *Build* pane via *More > Load All*, and the shortcuts Ctrl + Shift + L (Windows & Linux) or Cmd + Shift + L (macOS).
...
`R CMD check`, executed in the shell, is the gold standard for checking that an R package is in full working order. `devtools::check()` is a convenient way to run this:

FIXME: we need to beautify the `check()` output and just reveal less of it here.

```{r first-check-fake, eval = FALSE}
check()
```

```{r first-check, eval = create, warning = TRUE}
...
TIP: RStudio exposes `check()` via *Build > Check Package*, in the *Build* pane via *Check*, and the shortcuts Ctrl + Shift + E (Windows & Linux) or Cmd + Shift + E (macOS).
...
TIP: Use Ctrl + `.` in RStudio and start typing "DESCRIPTION" to activate a helper that makes it easy to open a file for editing. In addition to a filename, your hint can be a function name. This is very handy once a package has lots of functions and files below `R/`.
...
TIP: In RStudio, you can also do *Build > More > Document* or use a keyboard shortcut, Ctrl + Shift + D (Windows and Linux) or Cmd + Shift + D (macOS).
...
TIP: RStudio does similar via *Build > Install and Restart* and in the *Build* pane's *More > Clean and Rebuild*.
...
TIP: In RStudio you can also do *Build > Test package* or use the keyboard shortcuts Ctrl + Shift + T (Windows and Linux) or Cmd + Shift + T (macOS).
...
We're going to add another function to forcats that produces a sorted frequency table for a factor. We'll borrow some smarts from the forcats package, specifically the function `forcats::fct_count()`.
...
FIXME: make the `install()` output less ugly.
