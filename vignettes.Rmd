...
2.  Preview the rendered vignette periodically.
    ...
    Use `devtools::build_rmd("vignettes/my-vignette.Rmd")` in this case.
...
The rendered products (`.html`) are placed in `inst/doc/`, along with their source (`.Rmd`) and extracted R code (`.R`; discussed in @sec-vignettes-how-checked). Finally, when a package binary is made (@sec-structure-binary), the `inst/doc/` directory is promoted to a top-level `doc/` directory, as happens with everything below `inst/`.
