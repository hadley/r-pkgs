...
The technical distinction between a vignette (which ships with a package) and an article (which is only available on the website; see BLAH) is something the package developer needs to think about.
...
[^vignettes-2]: Sweave is the original system used for authoring vignettes (Sweave files usually have extension `.Rnw`).
    ...
    This choice also affects the form of rendered vignettes: Sweave vignettes render to PDF, whereas RMarkdown vignettes render to HTML.
...
This draft document has the the key elements of an R Markdown vignette and leaves you in a position to add your content.
...
2.  Render the entire vignette periodically.
    ...
    This requires some intention, because unlike tests, by default, your vignette is rendered using the currently installed version of your package, not with the current source package.
    ...
    It's very easy to over look this issue and be puzzled when your vignette preview doesn't seem to reflect recent developments in the package.
...
-   An external file that is (or could be) useful to users or elsewhere in the package (not just in vignettes): Put such a file in `inst/` (@sec-misc-inst), perhaps in `inst/extdata/` (@sec-data-extdata), and refer to it with `system.file()`.
    Example:
...
-   An external file whose utility is limited to your vignettes: put it in `vignettes/` and refer to it with a filepath that is relative to `vignettes/`.
...
Instead of a vignette, you can create an article, which is a term used by pkgdown for a vignette-like `.Rmd` document that is not shipped with the package, but that appears in the website.
...
This is problematic for a vignette, because the large size of the package causes problems with `R CMD check` (and CRAN) and is also burdensome for everyone who installs it, especially those with limited internet.
...
A better alternative for vignette inspection is use `devtools::build(vignettes = TRUE)` to create a package bundle and then look inside for the vignettes.
Or just build and install the package, with `devtools::install(build_vignettes = TRUE)`, and browse the vignettes.
...
In some domains, it might be impractical to rebuild vignettes as often our recommended workflow implies.
Here are a few tips:

-   You can prevent the cleaning of `inst/doc/` with `pkgbuild::build(clean_doc =)`.
    You can put `Config/build/clean-inst-doc: FALSE` in `DESCRIPTION` to prevent rcmdcheck from cleaning `inst/doc/` and something similar may be implemented for pkgbuild (https://github.com/r-lib/pkgbuild/issues/128).
