...
-   `Imports`: packages listed here *must* be present for your package to work.
    ...
    You'll learn about alternative ways to make functions in other packages available inside your package in section \@ref(imports).
...
[^dependencies-1]: The difference between loading and attaching a package is covered in more detail in \@ref(search-path).
...
-   `Depends`: Prior to the roll-out of namespaces in R 2.14.0 in 2011, `Depends` was the only way to "depend" on another package.
    ...
    You'll learn why, and when you should still use `Depends`, in [namespaces](#namespace).
...
[^dependencies-3]: The different package states, such as source vs. bundled, are explained in section \@ref(package-states).
