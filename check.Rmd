...
1.  Run `usethis::use_travis()` to set up a basic `.travis.yml` config file. 
...
R has recently become a community supported language on Travis and you can read the documentation at  <https://docs.travis-ci.com/user/languages/r/>.
...
`r_binary_packages`
 : A list of precompiled R packages to install from Ubuntu. This allows 
   ...
   package is available by searching on <https://packages.ubuntu.com> for 
...
To learn more, read about the many [deployment options](https://docs.travis-ci.com/user/deployment/) provided by Travis.
