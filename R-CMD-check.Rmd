...
Next the `DESCRIPTION` file is parsed and the package version and encoding are printed.
...
-   **Checking if there is a namespace**. You must have a `NAMESPACE` file. This is handled for you automatically by the devtools workflow.
...
But there are a few exceptions, i.e. there are a couple of NOTEs that you do not need to fix (and, indeed, probably can not fix).
