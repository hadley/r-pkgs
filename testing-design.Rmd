...
-   Runs test helper files, i.e. executes `tests/testthat/helper.R` (more on that below).
...
One other function that should almost never appear below `tests/testthat/` is `source()`.
...
Testthat has one more special file type: setup files, defined as any file below `tests/testthat/` that begins with `setup`.
