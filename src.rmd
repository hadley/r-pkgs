...
* `[[Rcpp::export]]`: makes the C++ function available to R.
...
To call a C function from R, you first need a C function! In R packages, C code lives in `.c` files in the `src/`. C functions. Your C function will need to work with R's data structures, so you'll need to include two header files:
...
To access this C function from R, we write an R function that uses `.Call()`:
...
Where does the first argument to `.Call()`, `add_`, come from? This is due to the `@useDynLib` tag which creates a line in the NAMESPACE that looks like this:

```
useDynLib(mypackage, add_)
```

This directive instructs R to create an object called `add_` that describes a function pointer:
...
`.Call()` takes pointer to a C function and calls it. It does not check the arguments (either number or type), so you need to do that in the R function, in the C function, or just accept that R will crash every time you accidentally supply the wrong type of input.
...
To register the add function defined above, we'd do:
...
To use access that function in another package, can `R_GetCCallable()`. It has two arguments, the function name and the package name, and it returns a function pointer. The function pointer has no type information, so it should always be wrapper in a helper function that verifies the inputs:
...
* If it uses the system descibed above, all you need to do is add
  ...
  add `#include mypackageAPI.h` in the C file.
