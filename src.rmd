...
1.  Click Build & Reload in the build pane, or press Cmd + Shift + B. You can
...
`.C()` is simpler than `.Call()` and can be useful if you already have standard C code. Since you never create R object in `.C()`, you never need to worry about memory management. To use it, you first write a void C function, using in-place modification of function parameters to return values:
...
1.  Modify the C code.
1.  Build and reload the package with Cmd + Shift + B
