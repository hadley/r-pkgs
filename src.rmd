...
1.  Create a new C++ file:
    ...
    ```{r, echo = FALSE}
    oldbookdown::screenshot("screenshots/new-cpp.png", dpi = 220)
    ```
...
* To speed up compilation on linux or mac, install `ccache`, then replace 
  `~/.R/Makevars` with:
    ...
    ```bash
    LTO=-O0 # is included after all other compilation flags
    MAKEFLAGS := -j 4 # use the number of processors available on your machine (GNU Make only)
...
When developing C or C++ code, it's usually better to use RStudio's Build & Reload instead of `devtools::load_all()`. If you have C objects that persist between reloads, and you change the data structure, then it's more than likely that R will abort when reloading the package. In addition, `devtools::load_all()` uses but does not update the object files (`*.o`) that are stored in `src/`, which means that the affected must be recompiled on each reload.
