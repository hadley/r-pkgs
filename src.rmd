...
Let's start with a simple C++ function with a common mistake:

```{r}
Rcpp::cppFunction("
NumericVector triple(NumericVector x) {
  int n = x.size();
  NumericVector out(n);
...
```
   frame #12: 0x00000001000a4bc6 libR.dylib`Rf_eval(e=0x0000000106050ae8, rho=0x0000000100d8d5b8) + 358 at eval.c:560
    ...
    frame #14: 0x00000001000a4ffd libR.dylib`Rf_eval(e=0x0000000100d8d3c0, rho=0x0000000100851f40) + 1437 at eval.c:676
    frame #15: 0x00000001000df12d libR.dylib`Rf_ReplIteration(rho=0x0000000100851f40, savestack=<unavailable>, browselevel=<unavailable>, state=0x00007fff5fbfe680) + 1101 at main.c:260
    frame #16: 0x00000001000e03c3 libR.dylib`run_Rmainloop [inlined] R_ReplConsole(rho=0x0000000100851f40, savestack=<unavailable>, browselevel=<unavailable>) + 97 at main.c:310
    frame #17: 0x00000001000e0362 libR.dylib`run_Rmainloop + 98 at main.c:1002
    frame #18: 0x0000000100000f3b R`main + 27
    frame #19: 0x00007fff93b2e5c9 libdyld.dylib`start + 1
```
