...
-   Turns on GitHub Pages for your repo and tells it to serve a website from the files found in the `gh-pages` .
...
But links from *other* people's packages (and websites, etc.) require two things:
...
For example, it's much easier to work with code that produces an error, it's less cumbersome to work with suggested packages, and there are ways to shield the reader from setup and teardown that does not apply to authentic usage.

A package's vignettes appear, in rendered form, in its website, in the "Articles" dropdown menu.
This is a colloquial use of "articles" that encompasses both formal vignettes, which are included in the package bundle, and non-vignette articles, which are available only on the pkgdown website.

In general, @sec-vignettes is our main source of advice on how to approach vignettes.
In this section we offer some additional advice on how to work with non-vignette articles (@sec-vignettes-article).
...
-   An example that will produce a lot of figures, which cause your package to bump up against CRAN's size constraints.

    The ggplot2 package [presents several FAQs as articles](https://ggplot2.tidyverse.org/articles/index.html) for this reason.

As with the reference index, there is a default listing of a package's articles (including vignettes).
But if your package has several articles, it can be worthwhile to organize them into groups.
For example, you might distinguish those that cover typical usage from those aimed at advanced users and developers.
