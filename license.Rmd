...
-   **Copyleft** licenses are stricter.
    ...
    Variations of the GPL include the [**LGPL**](https://choosealicense.com/licenses/lgpl-3.0/) which is more permissive and the [**AGPL**](https://choosealicense.com/licenses/agpl-3.0/) which is stricter[^license-1].

[^license-1]: It's sufficiently strict that many companies forbid the use of any AGPL software.
...
3.  Optionally, inspect the specific contributions and remove people who only contributed typo fixes and similar[^license-3].
...
[^license-3]: Very simple contributions like typo fixes are generally not protected by copyright because they're not creative works.
...
This is explicit in the [GitHub terms of service](https://docs.github.com/en/github/site-policy/github-terms-of-service#6-contributions-under-repository-license), but is generally considered to be true regardless of how the code is contributed[^license-4].
...
We only add core developers[^license-5] to the `DESCRIPTION` file; but some projects choose to add all contributors no matter how small.

[^license-5]: i.e. people responsible for on-going development.
