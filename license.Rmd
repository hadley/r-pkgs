...
Fortunately, if you're writing a package[^license-1] you don't need to be an expert to do the right thing: respecting how an author wants their code to be treated as indicated by the license they've picked.

[^license-1]: If, however, you're selling your package, I'd highly recommend that you consult a lawyer who specializes in open source software.
...
The R community is a little different: as of 2022, my analysis (following that of [Sean Kross](https://seankross.com/2016/08/02/How-R-Packages-are-Licensed.html)) found that \~70% of CRAN packages use a copyleft license and \~20% use a permissive license.
...
We'll finish off with a brief discussion of why we believe that simply using another package doesn't impose a licensing obligation.
...
We'll start by talking about code that you write, and how to license it to make clear how you want people to treat it/ It's important to use a license because if you don't the default copyright laws apply which mean that no one is allowed to make a copy of your code without your express permission.
...
There are three main cases[^license-2]:

[^license-2]: At least in the US; you'll need to double check with your local laws.
...
If multiple people have contributed to the package, there will be multiple copyright holders: each person or company will hold the copyright for their specific contribution.
...
It's important to think about your initial license because it's hard to change it later because it requires the permission of all copyright holders.
Unless you have a CLA, this will include everyone who has contributed a non-trivial amount of code.
...
3.  Optionally, inspect the specific contributions and remove people who only contributed typo fixes and similar[^license-5].
...
[^license-5]: Very simple contributions like typo fixes are generally not protected by copyright because they're not creative works.
...
-   If the code comes from Stack Overflow, it's licensed[^license-6] with the Creative Common CC BY-SA license, which is only compatible with GPLv3[^license-7]
...
[^license-7]: <https://creativecommons.org/share-your-work/licensing-considerations/compatible-licenses/>
...
It's our belief that simply using code from another package[^license-8] does not expose you to any licensing obligations.
When you import a package (e.g. ggplot2) and call a function (e.g `ggplot2::ggplot()`), none of the code of the code from that package is included in your package; it's not copied, and therefore copyright, the basis of open source licensing, doesn't come into play.
This is a very different situation to other languages, like C, because creating an C executable almost invariably ends up copying some component of the code you use into the executable.
Note, however, that any user of your package will have to download all the packages that your package depends on (as well as R itself), so will still have to comply with the terms of those licenses.

[^license-8]: i.e. packages that are listed in `Imports` or `Suggests` in your `DESCRIPTION`.
