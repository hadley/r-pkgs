...
But fortunately, if you're writing either an open-source package or a package used only within your organisation, you don't need to be an expert to do the right thing.
You just need to respect how an author wants their code to be treated as indicated by the license they've picked.
(If however, you're selling your package, we'd highly recommend that you consult a lawyer.)

This chapter will with an overview of licensing, and how to licensing your own code.
We'll then discuss the most important details of accepting code given to you (e.g. in a PR) and how to bundle code written by other people.
We'll finish off with a brief discussion of implication of using code from other packages.
...
The R community is rather different: as of 2022, my analysis[^license-1] found that \~70% of CRAN packages use a copyleft license and \~20% use a permissive license.
This means licensing your R package requires a little more care than for other languages.

[^license-1]: Inspired by that of [Sean Kross](https://seankross.com/2016/08/02/How-R-Packages-are-Licensed.html).
...
If you want to retain the ability to change the license, you need an explicit "contributor license agreement" or CLA, where the explicitly reassigns the copyright.
...
Does that mean your R code much always be GPL licensed?
...
This is also makes a difference if your R package has compiled code and you link to (using the `LinkingTo` in your `DESCRIPTION`): you'll need to do more investigation to make sure your license is compatible.
