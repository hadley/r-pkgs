...
#> [1] "2022-July-05_00-22-33_swim_clean.csv"
...
-   Copy `beach-lookup-table.csv` into the new package. But where? Let's try the top-level of the source package.
    -   This is not going to end well. Shipping data in a package is a special topic, which is covered in chapter \@ref(data).
...
Indeed, `R CMD check` fails for this package and we see this[^package-within-2]:

[^package-within-2]: If you have not done so already, you'll also need to configure a license to satisfy `R CMD check`.
    For a quick experiment like this, `usethis::use_mit_license()` is fine.
    For real work, see chapter \@ref(license) for guidance.

    -   installing *source* package 'delta' ... \*\* using staged installation \*\* R \*\* byte-compile and prepare package for lazy loading Error in library(tidyverse) : there is no package called 'tidyverse' Error: unable to load R code in package 'delta' Execution halted ERROR: lazy loading failed for package 'delta'
...
Here is the new version of `R/cleaning-helpers.R`[^package-within-3]:

[^package-within-3]: Putting everything in one file, with this name, is not ideal, but it is technically allowed.
...
It is also our strong recommendation that no one depend on the tidyverse meta-package in a package[^package-within-4].
Instead, it is better to identify the specific package(s) you actually use.
In this case, our package only uses dplyr.

[^package-within-4]: The blog post [The tidyverse is for EDA, not packages](https://www.tidyverse.org/blog/2018/06/tidyverse-not-for-packages/) elaborates on this.
...
We could add either of these lines to any file below `R/` to eliminate this note[^package-within-5]:

[^package-within-5]: For more details, see the [Programming with dplyr vignette](https://dplyr.tidyverse.org/articles/programming.html#eliminating-r-cmd-check-notes).
...
The heart of the timestamp strategy is this format string[^package-within-6]:

[^package-within-6]: `Sys.time()` returns an object of class `POSIXct`, therefore when we call `format()` on it, we are actually using `format.POSIXct()`.
    Read the help for [`?format.POSIXct`](https://rdrr.io/r/base/strptime.html) if you're not familiar with such format strings.


```r
format(Sys.time(), "%Y-%B-%d_%H-%M-%S")
#> [1] "2022-July-05_00-22-34"
```

This formats `Sys.time()` in such a way that it includes the month *name* (not number) and the local time[^package-within-7].

[^package-within-7]: It would clearly be better to format according to ISO 8601, which encodes the month by number, but please humor me for the sake of making this example more obvious.
...
```r
outfile_path("INFILE.csv")
#> [1] "2022-July-05_00-22-34_INFILE_clean.csv"

format(Sys.time(), "%Y-%B-%d_%H-%M-%S")
#> [1] "2022-July-05_00-22-35"
```
