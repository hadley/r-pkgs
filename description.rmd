...
* `Depends`: packages listed here require a specific version of R.
  ...
  play it safe and require a version greater than or equal to the version you're currently using. 
...
* `LinkingTo`: packages listed here rely on C or C++ code in another package. 
  Confusingly, this has nothing to do with linking but instead affects the 
  include path. You'll learn more about `LinkingTo` in [compiled code](#src).
