...
This command says both the the author (aut) and maintainer (cre) is Hadley Wickham, and his email address is `hadley@rstudio.com`. The `person()` function has four main arguments:
...
* A three letter code specifying the `role`. These are four important roles:
...
If you need to add additional clarification, you can also use the `comment` argument and supply arbitrary text.
...
Alternatively, you can specify this more concisely by using `as.person()`:
