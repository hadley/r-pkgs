...
It's even possible to use packages to structure your data analyses (see, e.g., @marwick2018-tas or @marwick2018-peerj), although we won't delve deeply into that use case here.
...
@sec-whole-game runs through the development of a small toy package.
...
@sec-setup describes how to prepare your system for package development, which has more requirements than simply running R scripts.
...
The basic structure of a package and how that varies across different states is explained in @sec-package-structure-state.

@sec-workflows101 goes over core workflows that come up repeatedly for package developers.
...
-   R code, @sec-r: the most important directory is `R/`, where your R code lives.
...
-   Package metadata, @sec-description: the `DESCRIPTION` lets you describe what your package needs to work.
...
-   Documentation, @sec-man: if you want other people (including future-you!) to understand how to use the functions in your package, you'll need to document them.
...
-   Vignettes, @sec-vignettes: function documentation describes the nit-picky details of every function in your package.
...
-   Tests, @sec-testing-basics: to ensure your package works as designed (and continues to work as you make changes), it's essential to write unit tests which define correct behaviour, and alert you when functions break.
...
-   Namespace, @sec-namespace: to play nicely with others, your package needs to define what functions it makes available to other packages and what functions it requires from other packages.
...
-   External data, @sec-data: the `data/` directory allows you to include data with your package.
...
-   Other components, @sec-misc: this chapter documents the handful of other components that are rarely needed: `demo/`, `exec/`, `po/` and `tools/`.

The final chapters describe general best practices not specifically tied to one directory:

-   Automated checking, @sec-r-cmd-check: R provides very useful automated quality checks in the form of `R CMD check`.
...
-   Release, @sec-release: the life-cycle of a package culminates with release to the public.
