...
Here's what actually happens when we try to run the data cleaning script:
...
In this book, we achieve this by putting `@export` in the special roxygen comment above each function (namespace management is covered in @sec-dependencies-NAMESPACE-workflow), like so:
...
Let's say that we add the `@export` tag to `localize_beach()`, `celsify_temp()`, and `outfile_path()` and run `devtools::document()` to (re)generate the `NAMESPACE` file, and re-install the delta package.
Now when we re-execute our script, it works!
...
Indeed, `check()` fails for this package and we see this:
...
And never again.
It is very easy to subconsciously assume your package code is re-evaluated when the package is attached or used.
But it is not.
...
The heart of the timestamp strategy is this format string[^package-within-4]:

[^package-within-4]: `Sys.time()` returns an object of class `POSIXct`, therefore when we call `format()` on it, we are actually using `format.POSIXct()`.
    Read the help for [`?format.POSIXct`](https://rdrr.io/r/base/strptime.html) if you're not familiar with such format strings.
...
This formats `Sys.time()` in such a way that it includes the month *name* (not number) and the local time[^package-within-5].

[^package-within-5]: It would clearly be better to format according to ISO 8601, which encodes the month by number, but please humor me for the sake of making this example more obvious.
...
Note that if you use withr as we do above, you would need to list it in `Imports` (@sec-dependencies-in-practice, @sec-dependencies-tidyverse).
