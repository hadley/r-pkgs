...
and one of its URLs links to a public hosting service, e.g.:
...
A __bundled__ package is a package that's been compressed into a single file. By convention (from Linux), package bundles in R use the extension `.tar.gz` and are sometimes referred to as "source tarballs". This means that multiple files have been reduced to a single file (`.tar`) and then compressed using gzip (`.gz`). While a bundle is not that useful on its own, it's a useful platform-agnostic intermediary between the other states. In the rare case that you need to make a bundle from a package you're developing locally, use `devtools::build()`. Under the hood, this calls `pkgbuild::build()` and, ultimately, `R CMD build`.

CRAN offers each package in this form via the "Package source" on the package's landing page. Continuing our examples from above, you could download the bundles `forcats_0.4.0.tar.gz` and `stringr_1.4.0.tar.gz`. You can unpack such a bundle in the shell (not the R console) like so:

```bash
tar xvf forcats_0.4.0.tar.gz
```

If you decompress a bundle, you'll see it looks almost the same as a source package. The main differences between an uncompressed bundle and a source package are:

* Vignettes are built, so rendered outputs, such as HTML, appear below
...
`.Rbuildignore` is a way to resolve some of the tension between the practices that support your development process and CRAN's requirements for submission and distribution. The affected files fall into two broad, semi-overlapping classes:
  ...
  * Files that help you generate package contents programmatically. Examples:
    ...
    - Using the [pkgdown package](https://pkgdown.r-lib.org) to generate a
      package website.
    - Storing `.R` scripts to create and update internal or exported data that
      ships in a package.
  * Files that drive package checking and documentation, outside of CRAN's
    purview. Examples:
    - Files consulted by your preferred development environment.
    - Configuration files related to continuous integration/deployment and 
      monitoring test coverage.

Here is a non-exhaustive selection of typical entries in the `.Rbuildignore` file for a package in the tidyverse:
...
^data-raw$          # Code used to create any data included in the package
