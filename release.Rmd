...
It's painful to manage multiple R versions, especially since you'll need to reinstall all your packages. Instead, you can run `R CMD check` on CRAN's servers with `devtools::check_win_oldrelease()`, `devtools::check_win_release()`, and `devtools::check_win_devel()`. Each of these functions builds your package and submits it to the CRAN win-builder with the latest __R-oldrelease__, __R-current__, or __R-devel__ versions. 10-20 minutes after submission, you'll receive an e-mail telling you the check results.

CRAN runs on multiple platforms: Windows, Mac OS X, Linux and Solaris. You don't need to run `R CMD check` on every one of these platforms, but it's a really good idea to do it on at least two. This increases your chances of spotting code that relies on the idiosyncrasies of specific platform. If you're on Linux or Mac, use `devtools::check_win_oldrelease()`, `devtools::check_win_release()`, and `devtools::check_win_devel()` to check on Windows. If you're on Windows, use Travis, as described in [continuous integration with Travis](#travis), to run checks on Linux.
