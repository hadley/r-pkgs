...
It's important to declare the release type up front, because the process (and, therefore, the checklist) looks different for, e.g., a patch release versus a major release.
...
Next comes a couple of items related to `R CMD check`.
...
This is just a last-minute, final reminder to re-check your package:

-   Locally, presumably with the current version of R, and with some extra checks that are usually turned off to make day-to-day development faster.

-   On CRAN's win-builder service, against a development version of R

Note that this list implicitly reflects that tidyverse packages are checked after every push via GitHub Actions, across multiple operating systems and versions of R, and that most of the tidyverse team develops primarily on macOS.
If your development process looks different from ours, here are more functions you might want to run before release:

-   `rhub::check_for_cran()`: Especially important if you are not using GitHub Actions to check regularly on the major CRAN platforms. Currently covers Windows and Linux.
-   `devtools::check_mac_release()`: Recommended if your primary development machine is not macOS.
-   `rhub::check_with_sanitizers()` and friends: Helpful for additional checks relevant to packages with compiled code.

### Reverse dependency checks {#sec-release-revdep-checks}

-   [ ] `revdepcheck::revdep_check(num_workers = 4)`[^release-2]

[^release-2]: Note that, at least at the time of writing, the revdepcheck package is not on CRAN.
...
In most cases, a change that affects revdeps is also likely to also break a lot of code that lives outside of CRAN packages, such as scripts, reports, and Shiny apps.

If you decide to proceed, you can use `revdepcheck::revdep_email()` to notify revdep maintainers *en masse*.
...
If you explain to CRAN that you gave advanced notice, they will accept your update, even if it breaks some other packages.
...
For example, [here](https://github.com/tidyverse/dplyr/issues/6262) is the issue where dplyr maintainers tracked hundreds of pull requests in the build-up to releasing dplyr v1.1.0.
...
2.  Reverse dependencies: If there are revdeps, this is where we paste the contents of `revdep/cran.md` (@sec-release-revdep-checks). If there are no revdeps, we recommend that you keep this section, but say something like: "There are currently no downstream dependencies for this package".
