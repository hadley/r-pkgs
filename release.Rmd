...
-   If necessary, update the check results and revep sections.
...
In this case, it is appropriate to send a reply email to CRAN explaining why you think these are false positive.
...
Note that `usethis::use_github_release()` depends crucially on the `CRAN-SUBMISSION` file that was written by `devtools::submit_cran()` and, after the successful creation of the GitHub release, it deletes this temporary file.
...
Now you're ready for the fun part: publicizing your package.
...
There are a number of places to announce your package.
If you have a blog, post about it there.
...
For package updates, remember that the existence of `NEWS.md` frees you from the need to describe every last change in your blog post.
...
Finally, it's a good idea to share the announcement via social media, such as on Twitter or Mastodon, using the #rstats hashtag.

TO DO: adding the blog post to pkgdown
