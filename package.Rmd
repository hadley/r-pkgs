...
Either route gets you to the same place: the smallest usable package, which has three components:
...
It also include an an RStudio project file, `pkgname.Rproj`, that makes your package easy to use with RStudio, as described below.

Don't use `package.skeleton()` to create a package. If you follow the workflow described in this work it creates more work you because it creates extra files that you'll have to delete or modify before you have a working package.
...
To make your first package, all you need to know is what you've learnt above. To master package development, particularly when you're distributing a package to others, it really helps to understand the five states of a package in its lifecyle: source, bundled, binary, installed and in-memory. Understanding the differences will help you form a better mental model of what `install.packages()` and `devtools::install_github()` do, making it easier to debug problems when they arise.
...
The only difference between `library()` and `require()` is what happens if the package isn't installed: `library()` will throw an error, where `require()` returns `FALSE` with a warning. Neither is useful when you're developing a package, because you have to install the package first. In the next chapter you'll learn about `devtools::load_all()` and RStudio's "Build and reload", which allow you to skip install and load a source package into memory:
...
A library is simply a directory containing installed packages. You can have multiple libraries on your computer and almost every one has at least two: one for packages you’ve installed, and one for the packages that come with every R installation (like base, stats, etc). Normally, that first directory varies based on the version of R that you’re using. That's why it seems like you lose all of your packages when you reinstall R --- they're still on your hard drive, but R can't find them. 
