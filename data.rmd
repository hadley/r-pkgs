...
The most common location for package data is (surprise!) `data/`. Each file in this directory should be a `.rda` file created by `save()` containing a single object (with the same name as the file). The easiest way to adhere to these rules is to use `devtools`::use_data()`:
...
It is possible to use other types of file, but I don't recommend it because `.rda` files are fast, small and explicit. Other options are described in `data()`.
