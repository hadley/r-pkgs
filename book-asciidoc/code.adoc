...
* Press Ctrl + `+.+` (the period) to bring up the _Go to File/Function_ tool, as shown in <<fig-go-to-file-function>>, then start typing the name. Keep typing to narrow the list and eventually pick a function (or file) to visit. This works for both functions and files in your project.
+
...
After navigating to a function with one of these methods, return to where you started by clicking the back arrow at the top-left of the editor (image:images/arrows.png[images/arrows,width=33,height=16]) or by pressing Ctrl + F9 (Windows & Linux) or Cmd + F9 (macOS).
