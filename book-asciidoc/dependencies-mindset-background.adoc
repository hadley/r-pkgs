...
These are basically regarded as free dependencies and can be added to `DESCRIPTION` via `usethis::use++_++tidy++_++dependencies()` (which also does a few more things). It should come as no surprise that these packages have a very small dependency footprint.
