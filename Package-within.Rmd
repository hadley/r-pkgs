...
*The section headers are an arbitrary selection of names related to bodies of water.
They appear in alphabetical order and their main purpose is to indicate the progression towards a working package.*

## Aegean: a script that works

Here is a fictional data analysis script for a group that collects reports from people who went for a swim:
...
In the absence of better information, they guess that Americans report temperatures in Fahrenheit and therefore those observations are converted to Celsius. (Yes, this seems to be too crude!)
...
## Baltic: a better script that works
...
```{cat, engine.opts = list(file = "Package-within-files/main-baltic"), R.options = list(tidyverse.quiet = TRUE)}
library(tidyverse)
...
```{r, code = readLines("Package-within-files/main-baltic"), R.options = list(tidyverse.quiet = TRUE)}
```
...
```{cat, engine.opts = list(file = "Package-within-files/cleaning-helpers-caribbean.R"), class.source = "R"}
library(tidyverse)
...
```{r, include = FALSE}
fs::file_copy(
  "Package-within-files/cleaning-helpers-caribbean.R",
  "cleaning-helpers.R"
)
```

```{cat, engine.opts = list(file = "Package-within-files/main-dardanelles"), R.options = list(tidyverse.quiet = TRUE)}
library(tidyverse)
...
```{r, code = readLines("Package-within-files/main-dardanelles"), R.options = list(tidyverse.quiet = TRUE)}
```
...
## Dardanelles: an attempt at a package
...
Concretely, we do this:
  ...
  * Copy `beach-lookup-table.csv` into the new package. But where? Let's try
    ...
    - This is not going to end well. Shipping data in a package is a special
      topic, which is covered in \@ref(data).
...
```{cat, engine.opts = list(file = "Package-within-files/main-dardanelles.R"), class.source = "R"}
library(tidyverse)
library(dardanelles)
...
None of our helper functions are actually available for use, even though we call `library(dardanelles)`!
...
In this book, we achieve this by putting `@export` in the special roxygen comment above each function (namespace management is covered in \@ref(namespace)).
...
We will fix this in our next iteration of the package (\@ref(data) has full coverage of how to include data in a package).
...
For a quick experiment like this, `usethis::use_cc0_license()` is fine.
For real work, see \@ref(license) for guidance.

```
* installing *source* package ‘dardanelles’ ...
** using staged installation
** R
** byte-compile and prepare package for lazy loading
Error in library(tidyverse) : there is no package called ‘tidyverse’
Error: unable to load R code in package ‘dardanelles’
Execution halted
ERROR: lazy loading failed for package ‘dardanelles’
* removing ‘/Users/jenny/rrr/dardanelles.Rcheck/dardanelles’
```
...
We discuss organising and naming the files below `R/` in FUTURE LINK TO CORRECT SECTION OF Code.Rmd.

```{cat, engine.opts = list(file = "Package-within-files/cleaning-helpers-erie.R"), class.source = "R"}
lookup_table <- tribble(
      ~where, ~english,
     "beach",     "US",
     "coast",     "US",
...
There are other ways to access functions in another package, explained in \@ref(namespace), but this is our recommended default.
...
The metadata conveyed via DESCRIPTION is covered in \@ref(#description).
...
We could add either of these lines to any file below `R/` to eliminate this note:
...
You've already seen how to create help files with roxygen comments in \@ref(whole-game-document) and we cover this topic thoroughly \@ref(man).
Therefore, we won't discuss this further here.

## Flores: build time vs. run time
...
## Gibraltar: side effects {#package-within-side-effects}

The timestamps now reflect the current time now, but the group raises a new concern.
...
colleagues <- tibble::tribble(
  ~location, ~LC_TIME, ~tz, 
  "Rome, Italy", "it_IT", "Europe/Rome",
  "Warsaw, Poland",  "pl_PL", "Europe/Warsaw",
  "Sao Paulo, Brazil",  "pt_BR", "America/Sao_Paulo",
  "Greenwich, England",  "en_GB", "Europe/London",
...
```{r echo = FALSE}
as_if(LC_TIME = "pt_BR", tz = "America/Sao_Paulo")
```
...
This (and `base::on.exit()`) are discussed further in FUTURE TO LINK TO CORRECT SECTION OF Code.Rmd.
...
I planned to follow up on the incorrectly converted temperatures, i.e. make the `if()` condition also include whether the reported `temp` is over a threshhold.
Now I'm not sure this is worth it.
Given then, the error we're making might just be distracting and should be removed.
