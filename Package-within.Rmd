...
```{cat, engine.opts = list(file = "Package-within-files/baltic-data-cleaning.R")}
library(tidyverse)
...
```{r, code = readLines("Package-within-files/baltic-data-cleaning.R"), R.options = list(tidyverse.quiet = TRUE)}
```
...
```{cat, engine.opts = list(file = "Package-within-files/caribbean-cleaning-helpers.R"), class.source = "R"}
library(tidyverse)
...
```{r, include = FALSE}
fs::file_copy(
  "Package-within-files/caribbean-cleaning-helpers.R",
  "cleaning-helpers.R"
)
```

```{cat, engine.opts = list(file = "Package-within-files/caribbean-data-cleaning.R")}
library(tidyverse)
...
```{r, code = readLines("Package-within-files/caribbean-data-cleaning.R"), R.options = list(tidyverse.quiet = TRUE)}
```
...
```{cat, engine.opts = list(file = "Package-within-files/dardanelles-main.R"), class.source = "R"}
library(tidyverse)
library(dardanelles)
...
```{cat, engine.opts = list(file = "Package-within-files/erie-cleaning-helpers.R"), class.source = "R"}
lookup_table <- tribble(
      ~where, ~english,
     "beach",     "US",
     "coast",     "US",
...
You've already seen how to create help files with roxygen comments in [the whole game chapter](whole-game-document) and we cover this topic thoroughly chapter \@ref(man).
Therefore, we won't discuss this further here.

## Flores: build time vs. run time {#package-within-build-time-run-time}
