...
During this time, you'll want to install the dependency from its development repository, which is often GitHub (but other remote sources are also supported).
...
Here's how the affected `DESCRIPTION` fields might look when depending on the development version of usethis:
...
It's important to note that you should not submit your package to CRAN in this state, meaning with a `Remotes˜` field and with a dependency required at a version that's not available from CRAN or Bioconductor.
For packages destined for CRAN, this is just a temporary development state, eventually resolved when the dependency updates on CRAN and you can bump your minimum version accordingly.

You may also see devtools-developed packages with `DESCRIPTION` fields that start with `Config/` or `Config/Needs/`.
...
<!-- TODO: Link to CI and pkgdown material when it exists. -->

The `Config/Needs/website` convention is handy because it allows a developer to use `DESCRIPTION` as their definitive record of package dependencies, while maintaining a clean distinction between runtime dependencies versus those that are only needed for specialized development tasks.
