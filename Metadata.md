...
You can see most of the structure in the simple example below.
...
* `Suggests`: your package can use these packages, but doesn't require them.
  ...
  your package.
  This means that you need to check if the package is available before using it.
  Do this with `requireNamespace(pkg, quietly = TRUE)`.
  ...
  rlang has some useful functions for checking if a package is available.
  Here's how suggested package checks could look if you use rlang:
    ...
    These rlang functions have handy features, such as vectorization over `pkg`,
    classed errors with a data payload, and, for `check_installed()`, an offer
    to install the needed package in an interactive session.
  
    `Suggests` isn't very relevant for packages used by a small number of people
    or in very specific context.
    Using `Suggests` is mostly a courtesy to your external users.
    It frees them from downloading rarely needed packages and lets them get
    with your package as quickly as possible.
...
You almost always want to specify a minimum version (`dplyr (>= 1.0.0)`) rather than an exact version (`dplyr (== 1.0.0)`).
...
If someone has an older package that doesn't have a function your package needs, they'll get an unhelpful error message.
However, if you supply a minimum version, they'll get a error message that tells them exactly what the problem is: an out-of-date package. 
