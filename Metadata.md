 ...
 * `Encoding` describes the character encoding of files throughout your 
   package. Our package development workflow always assumes that this
   is set to `Encoding: UTF-8` as this now the most commonly used text
   encoding, and we are not aware of any reasons to use a different value.
...
* `Version` is really important as a way of communicating where your 
  package is in its lifecycle and how it is evolving over time. 
...
## The `NAMESPACE` file {#namespace-NAMESPACE}
