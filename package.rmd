...
2. Choose "New Directory":
    ...
    ```{r, echo = FALSE}
    oldbookdown::screenshot("screenshots/create-1.png", dpi = 220)
    ```
   
2. Then "R Package":

    ```{r, echo = FALSE}
    oldbookdown::screenshot("screenshots/create-2.png", dpi = 220)
    ```

2. Then give your package a name and click "Create Project":

    ```{r, echo = FALSE}
    oldbookdown::screenshot("screenshots/create-3.png", dpi = 220)
    ```
...
*   You get useful keyboard shortcuts for common package development tasks.
    ...
    ```{r, echo = FALSE}
    oldbookdown::screenshot("screenshots/keyboard-shortcuts.png")
    ```
...
```{r, echo = FALSE}
oldbookdown::screenshot("screenshots/project-options-1.png", dpi = 220)
oldbookdown::screenshot("screenshots/project-options-2.png", dpi = 220)
```
...
Note that you cannot add comments directly into the `.Rbuildignore` file.
This would not work:

```
^.*\.Rproj$    # Automatically added by RStudio
```
Here is what the entries mean:

| regular expression    | glob               | purpose                                                |
|-----------------------|--------------------|--------------------------------------------------------|
| `^.*\.Rproj$`         | `*.Rproj`          | Automatically added by RStudio,                        |
| `^\.Rproj\.user$`     | `.Rproj.user`      | used for temporary files.                              |
| `^README\.Rmd$`       | `README.Rmd`       | An Rmarkdown file used to generate README.md           |
| `^cran-comments\.md$` | `cran-comments.md` | Comments for CRAN submission                           |
| `^NEWS\.md$`          | `NEWS.md`          | A news file written in Markdown                        |
| `^\.travis\.yml$`     | `.travis.yml`      | Used for continuous integration testing with Travis CI |
...
```{r, echo = FALSE}
oldbookdown::screenshot("diagrams/package-files.png")
```
...
```{r, echo = FALSE}
oldbookdown::screenshot("diagrams/installation.png")
```
...
```{r, echo = FALSE}
oldbookdown::screenshot("diagrams/loading.png")
```
