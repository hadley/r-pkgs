...
* `error = FALSE`: By default, snapshot code is *not* allowed to throw an error.
  ...
  But sometimes you want to assess is "Does this error message make sense to a
...
So pkgdown might as well call `testthat::is_testing(), instead of inlining it.

pkgdown lists testthat in Suggests, not Imports, so that is also interesting to me. Hmmm.
-->
...
At the very least, I think we need to point to packages that handle this and outline situations when mocking is one of the reasonable approaches.
...
* Test coverage should be "high", but going from 90% to 100% coverage is not
  always the best use of your development time and energy.
