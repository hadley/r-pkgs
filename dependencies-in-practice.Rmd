...
-   As close as possible to the usage of the external function.
    ...
    This starts to break down as soon as you use the external function in more than one place.
...
Another example is if you want to list an indirect dependency in `Imports`, so you can state a minimum version for it.
...
Our recommendation is to put a namespace-qualified reference (not a call) to an object in aaapkg in some file below `R/`, maybe in a `.R` file associated with package-wide setup:

```{r}
#| eval: false
ignore_unused_imports <- function() aaapkg::aaa_fun
```
...
### In test code {#sec-dependencies-suggests-in-tests}
...
### In examples and vignettes {#sec-dependencies-suggests-in-examples-and-vignettes}
...
In the devtools workflow and this book, we generate the `NAMESPACE` file from special roxygen comments in the `R/*.R` files.
The `NAMESPACE` file starts out with a single comment explaining the situation (and hopefully discouraging any manual edits):
...
The next time `devtools::document()` is executed, it will generate the appropriate directive in `NAMESPACE`, depending on the type of object (e.g. `export()` versus `S3method()` and so on).
In the example above, `NAMESPACE` would now look like:
