...
But if you install and attach the package with `library(pkg)`, you'll notice that no functions are available.
...
And NAMESPACE starts out with a single comment explaining the situation and hopefully discouraging any manual edits:
...
This will then generate the appropriate `export()`, `S3method()`, `exportMethods()`, or `exportClasses()` directive in `NAMESPACE`, depending on the type of the object.
...
For example, most of our packages have a `utils.R` file (@sec-code-organising) that contains many small functions that are useful internally, but aren't part of the core purpose of those packages.
We don't export such functions.
There are at least two reasons for this:

-   Freedom to not worry about robustness.
...
But we don't think you should release such a package.
