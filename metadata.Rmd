...
There are three other fields that allow you to express more specialised dependencies:
  ...
  * `Depends`: Prior to the rollout of namespaces in R 2.14.0, `Depends` was 
    ...
    `Depends: R (>= 3.4.0)`. Think carefully if you do this. On one hand,
    the safest thing to do is to require a version greater than or equal to
    your current R version. On the other hand, this prevents everyone with an
    older version of R from installing your package and may be unnecessarily
    conservative. The stakes are even higher if other packages depend on yours,
    because this causes a cascade of package installation failures.
