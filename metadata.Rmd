...
The `NAMESPACE` file advertises which functions your package makes available for others to use and, optionally, imports functions from other packages.

In this chapter, you'll learn about the most important fields found in `DESCRIPTION`.
The NAMESPACE file is covered in CROSSREF, where we INSERT DESCRIPTION OF DEPENDENCIES CHAPTER(S) HERE. The metadata part of the book concludes with @sec-license, which covers licensing.

## The `DESCRIPTION` file {#sec-description}
...
If you ever need to work with a `DESCRIPTION` file programmatically, take a look at the [desc package](https://r-lib.github.io/desc/), which usethis uses heavily under-the-hood.
...
-   The name, specified by the first two arguments, `given` and `family` (these are normally supplied by position, not name).
    ...
    For a non-person entity, such as "R Core Team" or "Posit PBC", use the `given` argument (and omit `family`).
...
If you add packages to `DESCRIPTION` with `usethis::use_package()`, it will also remind you of the recommended way to call them (explained more in CROSSREF).
...
Since R can't have multiple versions of the same package loaded at the same time, specifying an exact dependency dramatically increases the chance of conflicting versions[^metadata-2].

[^metadata-2]: The need to specify the exact versions of packages, rather than minimum versions, comes up more often in the development of non-package projects.
...
Literally, the `DESCRIPTION` file in the bundled package says `Depends: R (>= 3.5.0)`, even if `DESCRIPTION` in the source package says differently[^metadata-3].

[^metadata-3]: The different package states, such as source vs. bundled, are explained in @sec-package-states.
...
-   `Encoding` describes the character encoding of files throughout your package.
    Our package development workflow always assumes that this is set to `Encoding: UTF-8` as this is now the most commonly used text encoding, and we are not aware of any reasons to use a different value.
...
-   `SystemRequirements` is where you describe dependencies external to R.
    ...
    The most common usage is in the context of a package with compiled code, where `SystemRequirements` is used to declare the C++ standard or the need for GNU make.
    Examples:
...
## Custom fields {#sec-metadata-custom-fields}
...
We'll revisit this later when we explain how `Config/Needs/website` is used to record additional packages needed to build a package's website.
