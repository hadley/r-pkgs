...
If you use [git and github](#git), I highly recommend learning about [Travis](https://travis-ci.org/). Travis is continuous integration service, which means that it runs automated testing code everytime you push to github. For open source projects, travis provides 50 minutes of free computation on a ubuntu server. For an R package, the most useful code to run is `R CMD check`. 
...
* `./travis-tool.sh r_binary_install package`: installs an precompiled R 
  package from ubunutu. You can see if a binary version of a package is 
