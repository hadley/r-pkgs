...
An import part of the package development process is `R CMD check`. `R CMD check` automatically checks your code for common problems. It's essential if you're planning on submitting to CRAN, but it's useful even if you're not: it automatically detects many commons problems that you'd otherwise discover the hard way.

`R CMD check` will be frustrating the first time you run it - you'll discover many problems that you need to fix. The key to making `R CMD check` less frustrating is to run it more often. The sooner you find out about a problem, the easier it is to fix. The limit of this approach is to run `R CMD check` every time you make a change, and if you use github you'll learn how to do that in [Travis-CI](#travis).

## Workflow {#check-workflow}

`R CMD check` is the name of the command you run from the terminal. I don't recommend calling it directly, instead run `devtools::check()`, or press Cmd + Shift + E in RStudio. Compared to `R CMD check`, `devtools::check()`:

* Ensures that the documentation is up-to-date by first running 
  `devtools::document()`.

* Bundles the package before checking it. This the best practice for checking 
  packages because it makes sure the check starts from a clean slate: a package 
  bundle doesn't contain any of the temporary files that might have accumulated in
  your source package. For example, if you have compiled code, the source 
  package will include compilation artefacts like `.so` and `.o` files. These
  will cause spurious warnings, so should be avoided.

* Sets the `NOT_CRAN` environment variable to `true`. This allows you to 
  selectively skip tests on CRAN (see `?testthat::skip_on_cran` for more
  details).
...
`R CMD check` emits three types of message:
  ...
  * `ERROR`s: These are severe problems and you should fix regardless of whether
    or not you're submitting to CRAN.
    
  * `WARNING`s: These are likely problems. You must fix if you're planning to 
    submit to CRAN, and it's a good idea to look into them even if you're not.
    
  * `NOTE`s: These are the mildest of the `R CMD check` messages. 
  
    If you are submitting to CRAN, you should strive to eliminate all NOTEs, 
    even if they are false positives. If you have no NOTEs, human intervention 
    is not required, and the package submission process is easier.  If
    it's not possible to eliminate a `NOTE`, you'll need describe why it's ok
    in your submission comments, as described in [release notes](#release-check). 
    
    If you're not submitting to CRAN, carefully read each NOTE, but don't 
    go out of your way to fix things that you don't think are problems.

## Checks {#check-checks}

`R CMD check` is composed of over 50 individual checks, described in the following sections. For each check, I briefly describe what it does, what the most commons problems are, and how to fix them. When you have a problem with `R CMD check` and can't understand how to fix it, use this list to help. To make it easier to understand how the checks fit together, I've organised them into sections roughly corresponding to the chapters in this book. This means they will be in a somewhat different order to what you'll see when you run `check()`.
...
* __Checking if this is a source package__. You must check a source package, not 
  an binary or installed package. This should never fail if you use  
  `devtools::check()`.

<!-- https://github.com/wch/r-source/blob/trunk/src/library/tools/R/check.R#L3015 -->
* __Checking for executable files__. You must not have an executable files in
  ...
  each executable files in `BinaryFiles` field in your `DESCRIPTION`.)
...
* __Checking for portable file names__.  R packages must work on windows, 
  ...
  The easiest way to do this is to stick to letters, numbers and underscores 
...
* __Checking DESCRIPTION meta-information__. 
    ...
    * You should either provide `Authors@R` or `Authors` and `Maintainer`. 
      You'll get an error if you're specified both, which you can fix by 
...
* __Checking CRAN incoming feasibility__. These checks only apply if you're 
  submitting to CRAN.
    ...
    * If you're submitting a new package, you can't use the same name as 
...
You can run the most common of these outside `R CMD check` with `devtools::check_doc()` (which automatically calls `devtools::document()` for you). If you have documentation problems, it's best to iterate quickly with `check_doc()`, rather than running the full `R CMD check` each time. 
...
* __Checking tests__.  Each file in `tests/` is run. If you've followed the 
  ...
  Fix any failing tests by iterating with `devtools::test()`. 
    ...
    Occassionally you may have a problem where the tests pass when run 
...
If you use [git and github](#git), I recommend exploring the very useful [Travis](https://travis-ci.org/) continuous integration service. A continuous integration service runs code of your choosing every time you push a commit to github. For an R package, the most useful code to run is `R CMD check`. For open source projects, travis provides 50 minutes of free computation time on a ubuntu server - you can do anything you want on the server!
...
The travis config is a yaml file called `.travis.yml` and the default config looks like this:
...
Travis does not currently support R as a first class language, so we need to install it from scratch. We use the `travis-tool` script written by [Craig Citro](https://github.com/craigcitro/r-travis). It starts by downloading and running a script from github. This script provides a number of useful tools:
...
* `./travis-tool.sh dump_logs`: on failure, print out all relevant logs so
  you can easily read from 

There are two ways to install packages not included in the basic script:

* `./travis-tool.sh install_github rstudio/rmarkdown`: installs a package from
  github. 

* `./travis-tool.sh r_binary_install XML`: installs a ubunutu binary package.
  You can see if a binary version of your package is available by searching on
  <http://packages.ubuntu.com> for `r-cran-lowercasename`. For example, searching
  for `r-cran-xml` reveals that it exists,
  <http://packages.ubuntu.com/trusty/r-cran-xml>,
  
Both of these commands should be run before `install_deps`.
...
* `WARNINGS_ARE_ERRORS=1`: the travis build should fail if there are any
  warnings or errors (the default is to fail only on errors). I recommend
  that you set this if you plan to submit to CRAN.

* `_R_CHECK_FORCE_SUGGESTS_=0`: check even if all suggested packages can't be
  installed.
  
* `BOOTSTRAP_LATEX=1`: also install latex. This is useful if you want to 
  check the pdf manual or pdf vignettes, but adds substantially to the check
  time.

### Other uses

Since you can use Travis to run arbitrary code you can use it to implement arbitrary push-to-deploy strategies. For example, it's relatively easy to deploy code to S3 - this makes it easy (for example) to update a documentation website. For now, you'll need to figure it out yourself.
