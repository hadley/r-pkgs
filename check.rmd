...
`R CMD check` performs over 50 individual checks, described in the following sections. For each check, I briefly describe what it does, what are its most commonly observed problems, and how to fix those problems. When you have a problem with `R CMD check` that you ust can't fix, use this list to help. To make it easier to understand how the checks fit together, I've organised them into sections roughly corresponding to the chapters in this book. This means they will be in a somewhat different order to what you'll actually see when you run `check()`.

This list includes every check run in R 3.1.1. If you're using a more recent version, you may want to consult the most recent online version of this chapter: <http://r-pkgs.had.co.nz/check.html>. Please [let me know](mailto:hadley@rstudio.com) if you encounter a problem that this chapter doesn't address.

### Check metadata

`R CMD check` always starts by describing your current environment. I'm running R 3.1.1 on Mac OS X with a UTF-8 character set:
...
Next the package description is parsed and the package version number is printed. Here I'm checking httr version 0.5.0.9000 (you'll learn more about that weird version number in [versioning](#version)).
...
* __Checking package directory__. The directory you're checking must exist - 
  `devtools::check()` protects you from this trap.
...
* __Checking if this is a source package__. You must check a source package, not 
  a binary or installed package. If you use `devtools::check()`, you won't fail 
  this test.
...
* __Checking for executable files__. You must not have executable files in
  your package: they're not portable, they're not open source, and they're a
  security risk. Delete any executables files from your package. (If you're not 
  submitting to CRAN, you can silence this warning by listing each executable 
  file in `BinaryFiles` field in your `DESCRIPTION`.)

<!-- https://github.com/wch/r-source/blob/trunk/src/library/tools/R/check.R#L3103 -->
* __Checking for hidden files and directories__. On Linux and Mac OS X, files 
  with a name starting with `.` are hidden by default. So it's easy to include 
  them by mistake. You need to either delete them, or if they are important, 
...
* __Checking for portable file names__.  Because R packages must work on
  Linux, Mac OS X and Windows, you can only use file names that work on all 
  platforms. The easiest way to do this is to stick to letters, numbers, 
  underscores and dashes. You shoul also avoid non-English letters and spaces. 
  Fix any reported problems by renaming the listed files.
...
* __Checking for sufficient/correct file permissions__. If you can't read a 
  ...
  you have files in the package that you don't have permission to read. Fix these 
...
* __Checking whether package ‘XYZ’ can be installed__. `R CMD check` runs
  `R CMD install` to make sure it's possible to install your package. If 
  this fails, you should run `devtools::install()` or RStudio's Build & Reload
  to debug any problems before continuing.
...
* __Checking installed package size__. It's easy to accidentally include large
  ...
  whole package is less than 5 MB and that each subdirectory is less than 1 MB. 
  ...
  large file. 
    ...
    First, make sure the package is as small as it can possibly be: try 
    recompressing the data (see [data CRAN notes](#data-cran)), and minimising 
    vignettes (see [vignette CRAN notes](#vignette-cran)).
...
* __Checking top-level files__. Only specified files and directories are 
  ...
  To include other files, you have two choices:
    ...
    * If they need to be installed: move them to `inst/`. They'll be moved back 
      to the top-level package directory when installed.

<!-- https://github.com/wch/r-source/blob/trunk/src/library/tools/R/check.R#L924 -->
* __Checking package subdirectories__. 

    * Don't include empty directories. These are usually automatically removed  
      by `R CMD build` so you shouldn't see this error. If you do, however, just 
      delete the directories.
      
    * The letter case of files and directories is important. All sub-directories 
    ...
    * The contents of `inst/` shouldn't clash with top-level contents 
      of the package (like `build/`, `R/` etc). If they do, rename the 
...
* __Checking DESCRIPTION meta-information__. 
    ...
    * If you use any non-ASCII characters in the DESCRIPTION, you must also
      ...
      platforms: Latin-1, Latin-2 and UTF-8. I strongly recommend UTF-8:
      `Encoding: UTF-8`.
    ...
    * You should either provide `Authors@R` or `Authors` and `Maintainer`. 
      You'll get an error if you specify both. Fix by removing the one you 
      didn't want.
...
* __Checking package dependencies__. 
    ...
    * All packages listed in `Depends`, `Imports` and `LinkingTo` must be 
      ...
      dependencies is to run `devtools::install_deps(deps = TRUE)`.
  
    * Packages listed in `Suggest` must be installed, unless you've set 
      the environment variable `_R_CHECK_FORCE_SUGGESTS_` to FALSE (e.g. with 
    ...
    * Any packages used in the `NAMESPACE` must be listed in either
    ...
    * Every package listed in `Depends` must also be imported in the 
      ...
      `library(mypackage)`) but not when only loaded (e.g. `mypackage::foo()`)
...
* __Checking CRAN incoming feasibility__. These checks only apply if you're 
  submitting to CRAN.
    ...
    * If the maintainer of the package has changed (even if it's just an
      e-mail address), the new maintainer needs to submit it to CRAN, and the
      old maintainer should send a confirmation e-mail.
    ...
    * The `Title` and `Description` must be free of spelling mistakes. The 
      title of the package must be in title case. Neither the title nor the 
      description should include either the name of your package or the word 
      "package". Reword your title and description as needed.
...
* __Checking if there is a namespace__. You must have a `NAMESPACE` file.
  Roxygen2 will create one for you as described in [namespaces](#namespace).
...
* __Checking whether the package can be loaded with stated dependencies__.
  ...
  attached as usual). Failure here typically indicates that you're missing a 
...
* __Checking R files for non-ASCII characters__. For maximum portability (i.e.
  ...
  characters in R files. It's OK to use them in comments, but not in object 
  names. Strings should use Unicode escapes. See [`R/` CRAN notes](#r-cran) for 
  more details.
...
* __Checking dependencies in R code__. Errors here often indicate that you've 
  ...
  Alternatively, you may have accidentally used `:::` instead of `::` to access 
  an exported function from a package.

<!--- tools::checkS3methods --->
* __Checking S3 generic/method consistency__. S3 methods must have a compatible 
  function signature with their generic. This means that the method must have 
  the same arguments as its generic, with one exception: if the generic includes 
...
* __Checking R code for possible problems__. This is a compound check for 
    ...
    * `codetools::checkUsagePackage()` is called to check that your functions
      ...
      package functions, and hence avoid this NOTE. But if you must, see 
...
* __Checking contents of 'data' directory__.
    ...
    * If you've used a sub-optimal compression algorithm for your data, 
      re-compress with the suggested algorithm.n algorithm for your data, 
      re-compress with the suggested algorithm.

### Documentation

You can run the most common of these tests without resorting to `R CMD check` by using `devtools::check_doc()` (which automatically calls `devtools::document()` for you). If you have documentation problems, it's best to iterate quickly with `check_doc()`, rather than running the full `R CMD check` each time. 
...
* __Checking Rd cross-references__. Errors here usually indicate typos.
...
* __Checking examples__. Every documentation example must be able to run without errors, and do so in a reasonable amount of time. Exclude failing or slow tests with `\donttest{}`. See [documenting functions](#man-functions) for more details.
    ...
    parameter which tells it which function to start from. This way, once you've 
    discovered an error, you can rerun the check starting from just that file and 
    not all the files that lead up to it.
...
* __Checking PDF version of manual__. Occasionally you'll get an error when 
  ...
  bet is to look up the LaTex logs and associated Tex file and work back from 
...
* __Checking files in ‘vignettes’__. Problems here are usually straightforward - you've 
  included files that are already included in R (like `jss.cls`, `jss.bst`, or `Sweave.sty`), 
...
* __Checking for sizes of PDF files under 'inst/doc'__. If you're making PDF
  vignettes, make them as small as possible by running `tools::compactPDF()`.
...
* __Checking package vignettes in ‘inst/doc’__. This checks that every source
  ...
  [vignettes](#vignettes). If there is a problem, start by checking
  `.Rbuildignore`.
...
To run vignettes, the package must first be installed. That means `check()`:
...
If you use [git and GitHub](#git), I highly recommend learning about [Travis](https://travis-ci.org/). Travis is a continuous integration service, which runs automated testing code every time you push to GitHub. For open source projects, Travis provides 50 minutes of free computation time on an Ubuntu server. For an R package, the most useful code to run is `R CMD check`. 
...
1.  Wait a few minutes to see the results in your e-mail.

With this in place, every time you push to GitHub or someone submits a pull request, `R CMD check` is automatically run and you'll find out about failures right away, which makes them easier to fix. Using Travis also encourages me to run `R CMD check` locally more often, because I know if it fails I'll find out about it a few minutes later (often once I've moved on to a new problem).
...
Travis doesn't currently support R as a top-level language, so you'll need to install it. To do so, leverage Travis's built-in support for C (by specifying `language: c` in `.travis.yml`), and run the `travis-tool` script written by [Craig Citro](https://github.com/craigcitro/r-travis). The script downloads a number of useful tools from GitHub:
...
* `./travis-tool.sh dump_logs`: reports failures by printing out all relevant 
  logs in a form that is easily readable online.
...
* `./travis-tool.sh r_binary_install package`: installs an precompiled R 
  ...
  available by searching for `r-cran-lowercasename` on 
  <http://packages.ubuntu.com>. For example, searching for `r-cran-xml` reveals 
  that there's a binary version of XML package available.
...
* `BOOTSTRAP_LATEX=1`: also install LaTex. This is useful if you want to 
  check the PDF manual or PDF vignettes. It's not the default setting because it 
  adds substantially to the check time.

* `WARNINGS_ARE_ERRORS=1`: make the build fail if there are any `ERROR`s or 
  `WARNING`s. The default is to fail only on `ERROR`s. I recommending using
  this if you plan to submit to CRAN.

* `_R_CHECK_FORCE_SUGGESTS_=0`: runs the check even if all suggested packages 
  can't be installed.
  
### Other uses

Since Travis allows you to run arbitrary code, it has many other uses:

* Re-publishing a book website every time you make a change to the source code.
  (Like this book!)
