...
`R CMD check` is a system for automatically checking your code for common problems. It's essential for CRAN submission, but it's also useful if you're not planning on submitting to CRAN: it automatically detects many problems that you'd otherwise discover the hard way.
...
* `NOTE`: These are probable problems. Notes have some false positives,
  so if you're not planning on submitting to CRAN, you might not need to fix. 
  If you are submitting to CRAN you should attempt to remove all NOTEs, even 
...
2. Fix the first problem, and re-run `R CMD check`.

## Checks

The following sections describe each of the checks run by `R CMD check`. I describe what the check does, and how to fix the most common problems. I don't recommend studying this list exhaustively. Instead, when you have a problem with `R CMD check` and can't understand how to fix it, come back here and read up on potential solutions. To make it easier to find the check you're interested in, I've organised them into sections roughly corresponding to the chapters in this book. 

I've attempted to be exhaustive, but checks change with each version of R. If you're using a version of R later than 3.1.1, you may want to consult the most recent version of this chapter online: <http://r-pkgs.had.co.nz/check.html>.

### Check metadata

`R CMD check` always starts by describing the checking environment. I'm running R 3.1.1 on OS X with UTF-8 charset:
...
Next the description is parsed and the package version is printed. Here I'm checking the version 0.5.0.9000 (you'll learn more about that weird version number in [versioning](#version)) of the httr package.
...
-->
* __Checking contents of 'data' directory__.  Check valid extension.
...
* __Checking Rd metadata__. Checks that names and aliases are unique.
...
* __Checking Rd `\usage` sections__.  Checks that all arguments in funtions
  in usage section are documented (and vice versa). Checks that S3 and S4 
  methods use correct Rd markup
...
* __Checking examples__. Every documentation example must run without errors,
  ...
  See [documenting functions](#man-functions) for more details.
    ...
    Individual example shouldn't take more than 5s.
...
* __Checking installed files from ‘inst/doc’__.  You shouldn't have any files here.

<!-- https://github.com/wch/r-source/blob/trunk/src/library/tools/R/check.R#L1941 -->
* __Checking files in ‘vignettes’__. 
  Haven't included files "jss.cls", "jss.bst", "Rd.sty", "Sweave.sty"
  No leftover latex compilation files.
  Other stuff that shouldn't occur if you're using `vignettes/`.

<!-- https://github.com/wch/r-source/blob/trunk/src/library/tools/R/check.R#L2008 -->
* __Checking for sizes of PDF files under 'inst/doc'__. 
...
* __Checking package vignettes in ‘inst/doc’__. This checks that every source
  ...
  Checks makefile: case, line endings.
