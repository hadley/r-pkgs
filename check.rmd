...
<!-- https://github.com/wch/r-source/blob/trunk/src/library/tools/R/check.R#L3459 -->
* __Checking installed package size__. Packages should not be "too" big (<5 meg).
  Sub directories should be less than 1 meg.
...
-->
* __Checking CRAN incoming feasibility__.  
...
* __Checking whether the package can be unloaded cleanly__. Loads with 
  `library()` and then `detach()`es. Check `.onUnload()`.
...
* __Checking whether the namespace can be unloaded cleanly__.
  Checks `loadNamespace(); unloadNamespace()` works.
...
* __Checking loading without being on the library search path__.
  Calls `library(x, lib.loc = ...)`. Failure here probably indicates that you are 
  making some false assumption in `.onLoad` or `.onAttach`.
