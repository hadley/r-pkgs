...
* __Checking for portable file names__.  Because R packages must work on
  ...
  underscores and dashes. Avoid non-English letters and spaces. Fix any reported 
  problems by renaming the listed files.
...
* __Checking for sufficient/correct file permissions__. If you can't read a 
  ...
  by fixing the file permissions as needed.
...
* __Checking CRAN incoming feasibility__. These checks only apply if you're 
  submitting to CRAN.
    ...
    * If the maintainer of the package has changed (even if it's just the
...
You can run the most common of these tests without going to `R CMD check` by using `devtools::check_doc()` (which automatically calls `devtools::document()` for you). If you have documentation problems, it's best to iterate quickly with `check_doc()`, rather than running the full `R CMD check` each time. 
...
* __Checking examples__. Every documentation example must be able to run without errors, and do so in a reasonable amount of time. Exclude failing or slow tests with `\donttest{}`. See [documenting functions](#man-functions) for more details.
    ...
    parameter which tells it which function to start from. That way, once you've 
    discovered an error, you can rerun starting from just that file, not all the 
...
With this setup in place, every time you push to GitHub, and every time someone submits a pull request, `R CMD check` will be automatically run. You'll find out about failures right away. This makes them easier to fix. Using Travis also encourages me to run `R CMD check` locally more often, because I know if it fails I'll find out about it a few minutes later (often once I've moved on to a new problem).
...
Travis doesn't currently support R as a top-level language, so we need to install it. To do so, we start with an image designed for testing C projects; then we download the `travis-tool` script written by [Craig Citro](https://github.com/craigcitro/r-travis) that downloads a number of useful tools from GitHub:

* `./travis-tool.sh bootstrap`: installs R.

* `./travis-tool.sh install_deps`: installs all packages needed to run tests.

* `./travis-tool.sh run_tests`: builds the package and runs `R CMD check` on it.

* `./travis-tool.sh dump_logs`: reports failures by printing out all relevant 
  logs (easily readable online).
...
* `./travis-tool.sh r_binary_install package`: installs an precompiled R 
  ...
  that you can get a binary version of XML package.
...
* `BOOTSTRAP_LATEX=1`: also install LaTex. This is useful if you want to 
  check the PDF manual or PDF vignettes. It's not a default setting because it 
...
* Re-publishing a book website every time you make a change to its source code.
  (Like this book!)
