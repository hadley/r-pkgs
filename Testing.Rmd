...
Similarly, the `test-*.R` files below `tests/testthat/` should consist almost entirely of calls to `test_that()`.
...
* `source()` should not appear in your tests.
  ...
  betterways to make functions, objects, and other logic available in your tests.
...
Otherwise, consider making a file for test utilities, such as `R/utils-testing.R`.
...
We recommend maintaining teardown code with the setup code, in `setup.R`, which makes it easier to ensure they stay in sync.
