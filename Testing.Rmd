...
The test file name is displayed in testthat output, which provides helpful context[^testing-1].

[^testing-1]: The legacy function `testthat::context()` is superseded now and its use in new or actively maintained code is discouraged.
...
Here are the contents of `tests/testthat/test-dup.r` from stringr[^testing-2]:

[^testing-2]: Note that we are building the book against a dev version of stringr.
...
Here we deliberately constrain the width, in order to trigger a side-by-side layout.[^testing-3].
(We'll talk more about the withr package below.)

[^testing-3]: The actual waldo test that inspires this example targets an unexported helper function that produces the desired layout.
...
If nothing exists that meets your need, `withr::defer()` is the general way to schedule some action at the end of a test.[^testing-4]

[^testing-4]: Base R's `on.exit()` is another alternative, but it requires more from you.
...
If any of your setup should be reversed after test execution, you should also include the necessary teardown code in `setup.R`[^testing-5].
...
[^testing-5]: A legacy approach (which still works, but is no longer recommended) is to put teardown code in `tests/testthat/teardown.R`.
