...
1.  Add testthat to the `Suggests` field in the `DESCRIPTION`.
    Specifies testthat 3e in the `Config/testthat/edition` field.
...
You'll see this below when we discuss mechanisms for test setup / teardown and test helper files.
...
That is, writing good tests for a code base can sometimes feel more challenging than writing the code in the first place.
Even if it's not more difficult *per se*, testing presents many unique challenges and maneuvers, which tend to get much less air time in programming communities than strategies for writing the "main code".
...
* Global options, like `options()` and `par()`, and environent variables.
...
This is often referred to as *setup and teardown* and we'll describe various ways to do this, at various scopes.
...
We like to use the withr package (<https://withr.r-lib.org>) to make temporary changes in global state, because it automatically captures the initial state and arranges the eventual restoration.[^on-exit]
In this case, `withr::local_options(width = 20)` sets the `width` option to 20 and, at the end of the test, resets the option fo whatever its original value was.

<!-- Say something about:
Put withr in Suggests.
Call its functions like `withr::local_*()`.
Link back to the place where we outline our attitude towards unconditional use of Suggested packages in tests.
-->
...
withr has a large set of pre-implemented `local_*()` functions that should handle most of your testing needs.
Here a couple of the most useful functions:

* `local_options()` (see above)
* `local_envvar()` for temporarily setting an environment variable
    ```{r, eval = FALSE}
...
If nothing exists that meets your need, `withr::defer()` is the general way to schedule some action at the end of a test.

What about the things you need to do before you can execute your expectations?
...
Sometimes there's a certain type of setup that is needed across several tests.
You might be tempted to create an object in a test file, but outside of any specific test.
Due to environment chaining, that object is, indeed, visible inside the individual tests.
But the affected tests work more by coincidence, than by definition.

Here's a toy example to make things concrete.
Notice how `useful_thing` is defined outside of any test, then used in 2 subsequent tests.
...
If a `useful_thing` is costly to create, in terms of time or memory, do you really need to re-create it for each test run?
If not, then you can make it once, store it as a static test fixture, and load it in the tests that need it.
Here's a sketch of how this could look:

```{r eval = FALSE}
test_that("foofy() does this", {
  useful_thing <- readRDS(test_path("fixtures", "useful_thing.rds"))
  expect_equal(foofy(useful_thing, x = "this"), EXPECTED_FOOFY_OUTPUT)
})

test_that("foofy() does that", {
  useful_thing <- readRDS(test_path("fixtures", "useful_thing.rds"))
  expect_equal(foofy(useful_thing, x = "that"), EXPECTED_FOOFY_OUTPUT)
})
```

:::tip
`testthat::test_path()` is a handy function for building robust paths for files that live below `tests/testthat/`.
The resulting paths work during interactive development, when working directory is presumably set to the package root directory, and during all other methods of test execution, where the working directory could conceivably be different.
:::

Is it fiddly to create a `useful_thing`?
In that case, write a helper function to create instances of `useful_thing`:
...
and call the helper in the affected tests:

```{r eval = FALSE}
test_that("foofy() does this", {
  useful_thing <- new_useful_thing()
  expect_equal(foofy(useful_thing, x = "this"), EXPECTED_FOOFY_OUTPUT)
})
...
There are a couple of reasonable locations:
* `tests/testthat/helper.R`:
  Any `.R` file below `tests/testthat/` that starts with `helper` gets very
  special treatment by testthat and devtools.
  By default, such files are sourced by `load_all()` and when you run entire
  test file or the entire test suite.
* `R/utils-testing.R` or any other logical `.R` file below `R/`.
  An internal helper like `new_useful_thing()` might have uses beyond testing,
  e.g. behind-the-scenes in a vignette, and can simply be one of your package's
  unexported functions.
  Sometimes this makes you realize you should just export and document such a
  helper, so you can use it freely in documentation and tests.

<!-- Drafting this made me realize how hard it is to learn about the role of `tests/testthat/helper.*.[rR]` files. It's really not spelled out in any obvious way in the testthat docs. -->

A key feature shared by the locations described above is that the `new_useful_thing()` helper is available to you during interactive development after `load_all()` and during automated test runs.
Helper code that is lying around in other places, e.g. sprinkled around in test files, does not have this property.

If it's fiddly AND costly to create a `useful_thing`, the helper function could even use memoisation to avoid unnecessary re-computation.

Finally, remember, if a `useful_thing` can be created on-the-fly, without much fuss, then just inline it into the affected tests and don't worry about the repetition.
...
This is why we advocate inlining logic and objects, where practical.
When inlining is not practical, prefer making that object or logic available to all of your tests in a structured, explicit way.
The key point is to define the necessary object or function such that it's readily available after `devtools::load_all()`, `devtools::test_active_file()`, `devtools::test()`, etc.

Imagine yourself troubleshooting a test that's suddenly started failing on CRAN or on GitHub Actions.
If the test is self-sufficient, you can basically focus your attention to the universe contained within `test_that(desc, { ... })`.
The code inside `{ ... }` will either explicitly create all the necessary objects and conditions or make explicit calls to specific helpers or fixtures.
...
<!-- Do we need to explicitly say that a top-level call to `source()`, inside a test file, is even more beyond the pale? -->
