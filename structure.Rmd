...
This diagram summarizes the files present in the top-level directory for source, bundled, and binary versions of a fictional package named "zzzpackage" that uses most of the package parts discussed in this book.
...
@fig-package-files includes this comparison.
...
The following diagram illustrates the many ways a package can be installed, along with a few other functions for converting a package from one state to another.
...
For example, hopefully we'll be ready to recommend pak over install_github() before publication. -->
```
```{r}
#| label: fig-installation
#| echo: false
#| out-width: ~
#| fig-cap: Many methods for converting between package states.
#| fig-alt: > 
#|   A flow chart showing different routes that package installation can take: 
#|   1. 'R CMD install' can take either a package source, bundle, or binary and
#|      install it into the library;
#|   2. install.packages installs a package from CRAN from either binary or 
#|      source form. Binary packages are downloaded from CRAN and installed
#|      directly into the library using 'R CMD install'. If 'type = "source"' is
#|      specified, the package bundle is downloaded and installed via 'R CMD
#|      install', bypassing the binary state;
#|   3. devtools::install installs a source package via 'R CMD install', 
#|      bypassing bundle and binary states;
#|   4. devtools::install_github takes a package source that is on GitHub,
#|      downloads it and builds it into a bundle with devtools::build, then uses
#|      'R CMD install' to install from the bundle.
...
*2022-09-01 note: The pak package ([https://pak.r-lib.org)](https://pak.r-lib.org.) is gradually becoming our recommended tool for package installation and, most especially, for installing packages directly from GitHub. I.e. it is replacing `remotes::install_github()` It is likely that this recommendation will become official before the second edition of R Packages is published.*

<!-- TODO: Revisit this section with respect to pak. -->
