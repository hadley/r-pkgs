...
You'll learn more about the difference and why it's important in @sec-search-path.
...
In @sec-load-all, you'll learn how `devtools::load_all()` accelerates development by allowing you to load a source package directly into memory.

## Package libraries {#sec-library}

We just discussed the `library()` function, whose name is inspired by what it does.
When you call `library(pkg)`, R looks through the current **libraries** for an installed package named "pkg" and, if successful, it makes foo available for use.
...
[^structure-2]: Well, actually, `library()` loads and attaches a package, but that's a topic for @sec-search-path.
...
We explore this topic fully in @sec-metadata-namespace.
