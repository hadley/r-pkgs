...
The purpose of the namespace is to keep each package encapsulated and self-contained.
...
## When should you take a dependency? {#sec-dependencies-mindset}
...
6.  Functionality.
    ...
    It's easy to think "how hard can it be to write my own X?" when you are focused on the Happy Path[^dependencies-1].
    But a huge part of the value brought by packages like vctrs or tibble is letting someone else worry about edge cases and error handling[^dependencies-2]
...
[^dependencies-1]: In programming, the Happy Path is the scenario where all the inputs make sense and are exactly how things "should be".
...
[^dependencies-2]: Before writing your own version of X, have a good look at the bug tracker and test suite for another package that implements X.
...
Similar to optimizing performance, if you are worried about the burden of dependencies, it makes sense to address those concerns in a specific and quantitiave way.
...
-   `Imports`: packages listed here *must* be present for your package to work.
    ...
    Adding a package to `Imports` ensures it will be installed, but it does *not* mean that it will be attached along with your package, i.e. it does not do the equivalent of `library(otherpkg)`[^dependencies-4].
...
[^dependencies-4]: The difference between loading and attaching a package is covered in more detail in @sec-search-path.
...
So far, we've explained the mechanics of declaring a dependency in `DESCRIPTION` (\@sec-description-imports-suggests) and how to think about the costs and benefits of dependencies (@sec-dependencies-mindset). Before we can explain how to use your dependencies in various parts of your package, we need to establish the concepts of a package namespace and the search path.

https://github.com/hadley/r-pkgs/issues/204

Suraj Gupta's explanation of R lookup\
...
If you load lubridate, then here, `here()` will refer to the here version, because the last package attached wins.
But if you load the packages in the opposite order, `here()` will refer to the lubridate version.
...
The **imports** define how a function in one package finds a function it needs.
...
The **exports** help you avoid conflicts with other packages by specifying which functions are available outside of your package (internal functions are available only within your package and can't easily be used by another package).
...
### The `NAMESPACE` file {#sec-metadata-namespace}
...
There are other directives that we won't cover here, because they are explicitly discouraged or they rarely come up in our development work.

-   `exportPattern()`: exports all functions that match a pattern. Note that we feel it's better to always use explicit exports.
-   `exportClasses()`, `exportMethods()`, `importClassesFrom()`, `importMethodsFrom()`: export and import S4 classes and methods. We only engage in the S4 system when necessary for compatibility with another other package, i.e. we do not implement methods or classes that we own with S4. Therefore the S4 coverage in this book is very minimal.
...
We will have much more to say about roxygen comments and the roxygen2 package when we discuss package documentation in @sec-man. For now, we just lay out the reasons we prefer this method of generating the `NAMESPACE` file:
...
-   Roxygen2 keeps `NAMESPACE` tidy.
    ...
    Roxygen2 takes away the burden of writing NAMESPACE, but it also tries to make the file as readable as possible.
