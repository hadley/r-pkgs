...
One problem with simply minimizing the absolute number of dependencies is that it treats all dependencies as equivalent.
...
-   cli and glue, for creating a rich user interface
...
It is certainly true that many CRAN packages violate this threshhold (\>300 at the time of writing).
:::
...
-   `Imports`: packages listed here *must* be present for your package to work.
    ...
    Adding a package to `Imports` ensures it will be installed, but it does *not* mean that it will be attached along with your package, i.e. it does not do the equivalent of `library(otherpkg)`[^dependencies-2].
...
[^dependencies-2]: The difference between loading and attaching a package is covered in more detail in @sec-search-path.
