...
-   When should you take a dependency?
    ...
    This chapter also includes specific sections for deciding between `Imports` and `Suggests` (@sec-description-imports-suggests) and between Imports and `Depends` (@sec-dependencies-imports-vs-depends).
...
-   How should you use different kinds of dependencies in different contexts?
    ...
    We have to defer this to the next chapter (@sec-dependencies-in-practice), because the justification for those recommendations relies on some additional technical background that we share here.
...
[^dependencies-mindset-background-4]: The difference between loading and attaching a package is covered in more detail in @sec-dependencies-attach-vs-load.
...
`require(pkg)` is almost never a good idea[^dependencies-mindset-background-6] and, we suspect, may come from people projecting certain hopes and dreams onto the function name.
...
[^dependencies-mindset-background-6]: The classic blog post ["`library()` vs `require()` in R"](https://yihui.org/en/2014/07/library-vs-require/) by Yihui Xie is another good resource on this.

One place where we do use `require()` is to conditionally do something in `.Rprofile`.
For example, this snippet attempts to attach the reprex package in an interactive session, but fails quietly if reprex is unavailable:
