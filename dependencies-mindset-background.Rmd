...
-   `Imports`: packages listed here *must* be present for your package to work.
    ...
    Adding a package to `Imports` ensures it will be installed, but that is also all it does.
    It has nothing to do with actually importing or attaching that package.
    See @sec-dependencies-in-practice for more.
...
Another legitimate use of `require()` is in an example that uses a package your package `Suggests`, which is covered in @sec-dependencies-suggests-in-examples-and-vignettes.
