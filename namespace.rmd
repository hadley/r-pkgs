...
* __Loading__ will load code, data and any DLLs; register S3 and 
  ...
  package is available in memory, but you can't access its components 
  without using `::`. This is because the package is not in the search 
  path. A package is loaded by `::`, or you can load it explicitly with 
...
* __Attaching__ puts the package in the search path. You can attach
  ...
  function. Every attached package will be on the search path and will
  be listed by `search()`.
...
Now that you understand the importance of the namespace, let's dive into the nitty gritty details. The two sides of the package namespace, imports and exports, are both described by the `NAMESPACE`. You'll learn what this file looks like in the next section. In the following section you'll learn the details of exporting and importing functions and other objects.
