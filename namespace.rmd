...
Alternatively, if you are repeatedly using many functions from another package, you can import them with a single command using `@import package`. This is the least recommended solution because it makes your code harder to read (you can't tell where a function is coming from), and if you `@import` many packages, it increases the chance of conflicting function names.
