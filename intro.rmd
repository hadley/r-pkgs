...
In R, the package, a bundle of code, data, documentation and tests, is the fundamental unit of shareable code. But, as Hilary Parker puts in her [introduction to packages](http://hilaryparker.com/2014/04/29/writing-an-r-package-from-scratch/): "Seriously, it doesn't have to be about sharing your code (althought that is an added benefit. It is about saving yourself time." 

Packages save time because they are a set of conventions about how to organise your code. To realize that benefit, all you need to do is follow a template. The first convention is a no-brainer:
...
* You don’t need to think about the how to best organise a project.

* By following those conventions, you'll get access to many tools for free.
    
A package needn't be complicated. In fact, anytime you create a reusable set of functions you should put it in a package. Over time, you can add useful functions and features. To do that, you should follow the requirements for publishing packages on CRAN. Beyond putting your R code in the R/ directory, this means adding documentation in man/, vignettes in vignettes/, compiled code in src/, data sets in data/, and tests in inst/tests. 

While these requirements are strict and even though most packages won’t end up on CRAN, they're still very useful. For example, you can use packages to structure your data analyses, as Robert M Flight discusses in a [series of blog posts](http://rmflight.github.io/posts/2014/07/analyses_as_packages.html). More importantly, these conventions help make package development easy.

The most accurate resource for up-to-date details on package development is the official [Writing R Extensions][r-ext] guide. However, if you're not already familiar with the basics of packages that guide's rather hard to understand. This is because rather than focussing on the most commonly used components (as this book does), it's an exhaustive treatment which covers every possible aspect of writing packages. Once you've mastered the basics, Writing R Extensions will be a useful resource.
