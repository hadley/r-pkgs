...
I recommend that you always include `LazyData: true` in your `DESCRIPTION`, but only if you use a `data/` directory. If you include `LazyData: true` for a package that does not contain a `data` directory, `R CMD check` will generate a NOTE saying "'LazyData' is specified without a 'data' directory".
