...
* In a script, code is run when it is loaded. In a package, code is run when it
  is built. The top-level code that not inside any function will not be executed again in package loading, while functions and objects are saved in cache and can be reused. This means your package code should only create objects, the
