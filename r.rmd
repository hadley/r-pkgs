...
Congratulations, you've now understand the basics of using a package! In the rest of this chapter, you'll learn more about the various forms of a package, and exactly what happens when you run `install.packages()` or `install_github()`.

### Avoid side effects

One big difference between a script and R code in a package, is that the code in your package should have an side effects. Code in `R/` should only create objects (mostly functions). Outside of functions, you should not call functions that affect global state:
...
There are two reasons to avoid side-effects. The first is pragmatic: these funtions will work while you're developing a package locally using `load_all()`, but they won't work once you distribute your package. That's because during your R code is run once when the package is built, not everytime `library()` is called. The second is principled: you shouldn't change global state behind your users' backs.

Occassionally, packages do to perform side-effects. This is most common if your package talks to an external system - you might need to do some initial setup when the package loads. To do that, you can use the special functions `.onLoad()` and `.onAttach()`. These are called when the package is loaded or attached. You'll learn about the distinction between the two in [Namespaces](#namespace). Unless you know otherwise use `.onLoad()`. Whenever you use `.onLoad()` or `.onAttach()`, make sure to define `.onUnload()` or `.onDetach()` to clean up when the package is unload/detached. 

An older use of `.onAttach()` is to dynamically load a DLL file. This is no longer needed, and should be avoid. See [namespaces](#namespace) for details.  Can still be necessary if you're connecting to external C/C++ libraries that need set up and tear down. 
...
If you're planning on submitting your package to CRAN, you must use only ASCII characters in your `.R` files. You can still include unicode characters in strings, but you need to use the special unicode escape `"\u1234"` format:

```{r}
# TODO figure out how to display x as y: 
# http://stackoverflow.com/questions/25308913
x <- "•"
y <- "\u2022"
identical(x, y)
```
