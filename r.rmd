...
`create()` makes an `.Rproj` file for you. If you have an exisitng package that doesn't have a `.Rproj` file, you can use `devtools::use_rstudio("path/to/package")` to add it. If you don't use RStudio, you can get many of the benefits by starting a new R session and ensuring the working directory is set to the project directory. 
...
To make your first package, all you need to know is what you've learnt above. But to master package development, particularly when you're distributing a package to others, you'll need to understand the differences between the fives types of packages: source, bundled, binary, installed and in memory. Doing so will help you understand exactly what happens when you install a package with `install.packages()` or with `devtools::install_github()`.
