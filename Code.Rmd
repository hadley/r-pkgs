...
The next logical step is to collect such function definitions in one or more dedicated `.R` files and then `source()` them as needed.
...
Instead of re-running multiple `source()` commands, you can use `devtools::load_all()` to update all the function definitions in a package.
