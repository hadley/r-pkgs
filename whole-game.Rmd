...
We strongly suggest you follow along and create this toy package with us, however if you just want to read through it (or something goes awry halfway through!), completing it is not critical to understanding the remainder of the book.
...
The functions we use make sure that the package we will create is in a new, clean, project.
...
We call the package **regexcite** and it will have a couple of functions that make common tasks with regular expressions easier.
Please note that these functions are very simple and definitely not the point of this exercise!
...
See @sec-workflow101-create-package for more.
...
-   `.Rbuildignore` lists files that we need to have around but that should not be included when building the R package from source. By default, this file (as well as `.gitignore`) is only created by `create_package()` if you are using RStudio, however you will likely need to make use of it in the future regardless of what editor you are using. It is discussed in more detail in @sec-rbuildignore.
...
See @sec-workflow101-load-all for more.
