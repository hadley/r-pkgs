...
```{r use-dplyr, eval = FALSE}
use_package("dplyr")
```

```{r commit-dplyr-imports, echo = params$debug, comment = NA, eval = FALSE}
paths <- unlist(status(repo))
add(repo, path = paths)
...
```{r freq-out-fodder, asis = TRUE, echo = FALSE, comment = NA, eval = FALSE}
freq_out_fodder <- c(
...
```{r freq-out-test-drive, eval = FALSE}
load_all()
freq_out(iris$Species)
```
...
```{r document-freq-out, eval = FALSE}
document()
```
...
```{r pkg-doc-fodder, asis = TRUE, echo = FALSE, comment = NA, eval = FALSE}
...
```{r pre-readme-install, eval = FALSE}
install()
```
...
```{r cat-readme-rmd, asis = TRUE, echo = FALSE, comment = NA, include = FALSE, eval = FALSE}
...
```{r render-readme-rmd, include = params$debug, eval = FALSE}
...
```{r teardown, include = params$debug, eval = FALSE}
...
```{r include = params$debug, eval = FALSE}
