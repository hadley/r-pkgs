...
RStudio can initialize a Git repository, in any Project, even if it's not an R package, as long as you've set up RStudio + Git integration.
...
Because this comes up fairly often in real life, we have the `rename_files()` function, which choreographs the renaming of a file in `R/` and its associated companion files below `tests/`.
