...
The goal of this chapter is to help you graduate from using informal ad hoc testing, done at the command line, to formal automated testing (aka unit testing). While turning casual interactive tests into reproducible scripts requires a little more work up front, it pays off in four ways:
...
* A __test__ groups together multiple expectations to test the output
  ...
  across multiple functions. A test is created with `test_that()` .
...
I'd start by defining a couple of helper functions to make each expectation more concise. Then each test can fit on one line, and you can line up actual and expected values to make it easier to see the differences:
