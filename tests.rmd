...
Each line represents a file of tests. Each `.` represents a passed test. Each number represents a failed test. The numbers index into a list of failures that provides more details:
...
* A __test__ groups together multiple expectations to test the output
  ...
  across multiple functions. A test is created with the `test_that()` function.
...
An expectation is the finest level of testing. It makes a binary assertion about whether or not a value is what you expect. All expectations have a similar structure:
...
While you'll normally put expectations inside tests inside contexts, you can also run them directly. This makes it easy to explore expectations interactively. There are almost 20 expectations in the testthat package. The most important are discussed below.
...
Each test is run in its own environment and is self-contained. The exception are actions which have effects outside the local environment. These include things like:
...
There is a fine balance to writing tests. Each test that you write makes your code less likely to change inadvertently; but it also can make it harder to change your code on purpose. Focus on testing the external interface to your functions - if you test the internal interface, then it's harder to change the implementation in the future because as well as modifying the code, you'll also need to update all the tests.
...
Instead you can use a little [non-standard evaluation](http://adv-r.had.co.nz/Computing-on-the-language.html) to produce something more informative. The key is to use `bquote()` and `eval()`. In the `bquote()` call below, note the use of `.(x)` - the value of anything inside that argument will be inserted into the call.
...
This sort of refactoring is often worthwhile because removing redundant code makes it easier to see what's changing. More importantly, readable tests give you more confidence that they're correct.
...
The highest-level structure of tests is the file. Each file should contain a single `context()` call that provides a brief description of its contents. Just like the files in the `R/` directory, you are free organise your tests any way that you like. But again, the two extremes are clearly bad (all tests in one file, one file per test). You need to find a happy medium that works for you. A good starting place is to have one file of tests for each complicated function.
