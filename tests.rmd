...
The goal of this chapter is to help you graduate from informal tests at the command line to formal automated tests using the testthat package. Turning your casual interactive tests into reproducible scripts requires a little more work up front. Since you can no longer visually inspect the output, you have to write code that does the inspection for you. However, this is an investment in the future of your code. It pays off in four ways:
...
A test file looks like this: (this is a real example from the stringr package)
...
While you'll normally put expectations inside tests inside texts, you can run them directly. This makes them easy to explore interactively. There are almost 20 expectations in the testthat package. The most important are discussed below.
...
*   `expect_equal()` uses `all.equal()` to check for equality within some 
    ...
    exotic object like an environment, use `expect_identical()`. It uses 
    ...
    On the other hand, `is_equivalent_to()` is a more relaxed version of 
    `equals()` that ignores attributes:
...
*   `expect_match()` is used by three expectations that check for various 
    ...
    live the second argument blank if you just want to see if a message,
    warning or error is throw. However, it's normally better to be explicit.
...
*   Sometimes you don't know exactly what the result should be, or it's too 
    complicated to easily recreate. In that case the best you can do is check
    that the result is the same as last time. `expect_equal_to_reference()` 
...
Not that this won't produce very useful error messages. Instead you can use a little [non-standard evaluation](http://adv-r.had.co.nz/Computing-on-the-language.html) to produce more informative output:

```{r, error = TRUE}
expect_time <- function(x, y) expect_equal(x, as.POSIXct(y, tz = "UTC"))
expect_time(Sys.time(), "2009-01-01 00:00:00")

expect_time <- function(x, y) eval(substitute(expect_equal(x, as.POSIXct(y, tz = "UTC"))))
expect_time(Sys.time(), "2014-01-01 00:00:00")
```
