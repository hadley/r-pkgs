...
Some developers never commit derived files, files that can be generated automatically. For an R package this would mean ignoring the `NAMESPACE` or `man/` directories because they're generated from comments. But from a practical pespective, it's better to commit these files. Because R packages have no way to generate `.Rd` files on installation, ignoring these files means that users who install your package from GitHub will have no documentation.
