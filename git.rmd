...
You need to work through each conflict and decide either which version is better, or how combine the changes from both versions. Make sure you delete the conflict markers to resolve the conflict, then stage the file. Once you've fixed all conficts, make a new commit and push to github.
...
Github has some good documentation on using <https://help.github.com/articles/using-pull-requests/>. In this chapter, I'll focus on the minimum you need to know to use pull requests effectively, and show you how they tie in to the git commands you've learned so far.
...
Recieving a pull request is fantastic. Someone not only cares about your package enough to use it, they've actually read the source code and make an improvement!
...
If the button doesn't work, github provides some instructions to do it from the command line:

1. `git add remote` + `git fetch`: get's a local copy of their branch on your 
    computer.
    
1. `git merge <branch>` merges their changes into master.

1. You resolve the conflicts, then stage, commit and push like usual.
