...
This is not to say that Git is easy to learn. You initial experiences with git are likely to be frustrating and you will frequently curse at the strange terminology and unhelpful error messages. Fortunately, there are many tutorials available online, and while they aren't always well written (many provide a lot information but little guidance about what to do with it or why you need to care), you can absolutely can master git with a little practice. Don't give up! Persevere and you'll unlock the super-power of code collaboration.  
...
1.  If needed, generate a SSH key. SSH keys allow you to securely 
    ...
    For your own personal computer, it's much more convenient and only slightly
    less secure to not use a passphrase, so I recommend against it.
...
The fundamental unit of work in Git is a __commit__. A commit takes a snapshot of your code at a specific point in time. If you make a mistake, you can undo your changes by rolling back to a previous commit. Coding with commits is like using an anchor when rock climbing. If you're crossing a dangerous rock face you want to make sure you've got something to catch you if you fall. Commits play a similar role: if you make a mistake, you can't fall past the previous commit. Coding without commits is like free-climbing: you can travel much faster in the short-term, but in the long-term the chance of a catastrophic failure are high! However, you need to be judicious in your use of commits (and anchors). Because of the time and effort involved, committing too frequently will slow your progress. But when you're in uncertain or dangerous territory, you should actually use more commits. Also as a document of your efforts, commits are helpful to others: they show your journey, not just the destination.

There are five key components to every commit:

* A unique identifier, called an SHA (short for secure hash algorithm).

* A changeset that describes which files were added, modified or deleted.

* A human-readable commit message.

* A parent, the commit that came before. (There are two exceptions to this rule: 
  the initial commit doesn't have a parent, and merges, which you'll learn about 
  later, have two parents.)
...
The commit window consists of three panes:
...
* The top-right pane is where you'll enter the commit message, a human 
  readable message summarising the changes. More on this shortly.
...
1.  __Write a commit message__ (top-right panel) which describes the changes
    ...
    line. It should be brief (50 characters or less). For complicated commits, 
    you can follow it with a blank line and then a paragraph or bulleted list 
    providing more detail. The convention is to write messages in the imperative
    (like you're telling someone what to do): "fix this bug", not "fixed this bug" 
    or "this bug was fixed".
...
1.  It's easy to see the history of a file. If you navigate to a file and click 
    ...
    helpful when you're tracking down a bug. You can jump directly to these pages 
    from RStudio with the Git dropdown in the main toolbar: 
...
It's useful to create your own new branches when you want to (temporarily) break away from the main stream of development. You can create a new branch with `git checkout -b <branch-name>`. Names should be in lower case letters and numbers, with `-` used to separate words. 
...
Below, you'll learn about pull requests in the context of your own code. This may seem a bit pointless because you don't _need_ them, as you can directly modify your code. But pull requests are surprisingly useful because they allow you to get feedback on a proposed changes. In fact, we use them frequently at RStudio to get feedback before merging major changes.
