...
Although you haven't realised it, you've been using two branches so far: master and origin/master: master contains all your commits, origin/master contains all the commits on github (from the last time that you synced). When you run `git pull`, it does two things:
...
It's useful to create your own new branches when you want to break off the main stream of development for a while. To do this, you'll need to know:

1.  How to create new branches, see what branches are available and delete them 
    when you're done. How to switch between branches.
1.  How to pull the changes from one branch into another, so that
    you can keep a side branch up-to-date with development in the main
    branch.
1.  How to synchronise a branch with github, and how to install a package
    from a branch.

### Creating, switching, listing and deleting

* Create a new branch: `git checkout -b <branch-name>`. Names should use
  lower case letters, numbers, and `-` to separate words
  
* List all existing branches with `git branch`.  Use `git branch -a` to 
  also see the remote branches that you have synched locally.

* Switch between branches with `git checkout <branch-name>`. You can always 
  return to the main line of development with `git branch master`. You can 
  also use the branch switcher at the top right of the git pane:
  
    ```{r, echo = FALSE}
    bookdown::embed_png("screenshots/git-branch.png", dpi = 220)
    ```
  
* Once you're done with a branch, delete it with `git branch -d <branch-name>`.
  Git won't let you delete it if you haven't merged it back in to the master
  branch. If you want to force delete it, use `-D` instead of `-d`.
