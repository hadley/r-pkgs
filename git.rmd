...
If you've never used the shell before, don't worry, it's very similar to using R. The main difference is that instead of functions, you call commands. Commands have a slightly syntax (unfortunately shell commands are even less regular than R functions, but you'll only need a couple). For example, in R you might write `f(x, y = 1)`, where in the shell you'd write `f x --y=1` or `f x -y1`. In this chapter, you won't be doing much in the shell apart from running the git commands that I give you. However, it's a good idea to learn the three most important shell commands:
...
## Add and commit files {#git-stage-commit}

The fundamental unit of work in git is a __commit__. A commit takes snapshot of your code at a specified point in time. If you make a mistake, you can rollback changes to a previous commit. To see the history of a package, you can compare each commit to the previous to see what you've done. Most importantly for collaboration, you can combine commits from multiple people. 

There are five components to every commit:
...
The staging area lets you review your changes prior to committing them. It's a useful place to double check your code.
...
1.  __Select files__.  To stage (select) a single file for inclusion in the next
    commit, tick the corresponding check box. To stage all files, press Cmd + A, 
    ...
    You'll see two new new icons when staging files:
