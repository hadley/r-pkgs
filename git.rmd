...
It's useful to create your own new branches when you want to break off the main stream of development for a while. 

Basic branch management tools.
...
* List all existing branches with `git branch`. Use `git branch -a` to 
  also see the remote branches that you have synced locally.
...
Once you're done working in a branch, you can merge it back into master and then delete it:
...
If you want to abandon a branch (i.e. delete it without merging it), you need a slightly different commnand. If you want to force delete it, use `-D` instead of `-d`.

If you go to synchronise this branch to github, you'll notice that push and pull are disabled: `r bookdown::embed_png("screenshots/git-no-remote.png", dpi = 220)`. To enable them, you'll need to first tell git that your local branch should have a remote equivalent:
