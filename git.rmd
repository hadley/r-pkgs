...
The purpose of the commit message is that when other people (including future you!) are skimming the commits, your commit describes what has changed. It's worthwhile to spend a little effort on your commit messages, so that if you do need to look at history, you can easily digest it. But don't spend too much time!  This is especially important when you're on a team.

Good commmit messages help other people understand the history of your project. The most important other person is future you, who probably has forgotten a lot of things that present you knows. There's nothing more frustrating than tracking down a new bug to line of a code and looking up the commit message only to find that it says "fix bug".
...
Smaller commits make it easier to work with others. If two people change the same file in the same place, you'll only need to resolve a small problem instead of a big one.
