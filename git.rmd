...
1.  If needed, generate a SSH key-pair. SSH keys are used to securely communicate
    ...
    You can check if you already have an SSH key-pair by running in R:
...
1.  It's easy to see the history of a file. Navigate to the file, then click
    ...
    each line of code. This is tremendously helpful when you've discovered a bug 
    and want to understand its history.
...
1.  Is it polished? In the final review phase, make sure that the non-code
    parts of the PR are polished. Prompt the contribute to update the 
    documentation, point out spelling mistakes and suggest better wording.
    
    I recommned asking the contributor to include a bullet point in `NEWS.md`, 
