...
Some developers never commit derived files, files that can be generated automatically: for an R package this would mean never checking in the `NAMESPACE` or `man/` directory  because they're generated from comments. But from a practical pespective, it's better to commit these files: because R packages have no way to generate `.Rd` files on installation, users who install your package from GitHub will have no documentation.
