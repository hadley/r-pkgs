...
* In the middle, the code from the previous commit.
* At bottom, the code that you're trying to merge in.

You need to work through each conflict block and decide either which version is better, or integrate the changes from each.  Delete the conflict markers, leaving only the new code.  As you fix each file, stage it, and then once you've fixed them all, make a new commit. 
