...
1.  If needed, generate a SSH key. SSH keys allow you to securely 
    ...
    ```{r, echo = FALSE}
    oldbookdown::embed_png("screenshots/git-config-2.png", dpi = 220)
    ```
...
To create a new repo:
  ...
  * In RStudio, go to project options, then to the Git/SVN panel. Change the 
    ...
    ```{r, echo = FALSE}
    oldbookdown::embed_png("screenshots/git-proj-config.png", dpi = 220)
    ```
...
Once Git has been initialised, you'll see two new components:
  ...
  * The __git pane__, at the top-right, shows you what files have 
    ...
    ```{r, echo = FALSE}
    oldbookdown::embed_png("screenshots/git-pane.png", dpi = 220)
    ```
  ...
  * The __git dropdown__ menu, found in the toolbar, includes Git and GitHub commands
    ...
    ```{r, echo = FALSE}
    oldbookdown::embed_png("screenshots/git-dropdown.png", dpi = 220)
    ```
...
* `r oldbookdown::embed_png("screenshots/git-modified.png", dpi = 220)`, 
  __Modified__. You've changed the contents of the file.

* `r oldbookdown::embed_png("screenshots/git-unknown.png", dpi = 220)`, 
  __Untracked__. You've added a new file that Git hasn't seen before.

* `r oldbookdown::embed_png("screenshots/git-deleted.png", dpi = 220)`, 
...
```{r, echo = FALSE}
oldbookdown::embed_png("screenshots/git-diff-window.png", dpi = 220)
```
...
```{r, echo = FALSE}
oldbookdown::embed_png("screenshots/git-commit-window.png", dpi = 220)
```
...
(Yes, this is exactly the same window you see when clicking `r oldbookdown::embed_png("screenshots/git-diff.png", dpi = 220)`!)
...
1.  __Open the commit window__ by clicking 
    `r oldbookdown::embed_png("screenshots/git-commit.png", dpi = 220)` or 
    pressing `Ctrl + Alt + m`.  
...
1.  __Select files__.  To stage (select) a single file for inclusion, tick its
    ...
    `r oldbookdown::embed_png("screenshots/git-stage.png", dpi = 220)`. 
    ...
    and you might see one of two new icons:
      ...
      * Added: `r oldbookdown::embed_png("screenshots/git-added.png", dpi = 220)`: 
        after staging an untracked file, Git now knows that you want to add it 
        to the repo.
      
      * Renamed: `r oldbookdown::embed_png("screenshots/git-renamed.png", dpi = 220)`:
    ...
    `r oldbookdown::embed_png("screenshots/git-modified-staged.png", dpi = 220)`.
...
```{r, echo = FALSE}
oldbookdown::embed_png("screenshots/git-ignore.png", dpi = 220)
```
...
*   To undo the changes you've just made, right click on the file in the Git 
    ...
    undo:`r oldbookdown::embed_png("screenshots/git-chunk.png", dpi = 220)`. You 
...
*   If you committed changes too early, you can modify the previous commit by 
    ...
    `r oldbookdown::embed_png("screenshots/git-commit-amend.png", dpi = 220)`. 
...
1.  Open the history window by clicking
    `r oldbookdown::embed_png("screenshots/git-history.png", dpi = 220)` in the 
    Git pane.
    
    ```{r, echo = FALSE}
    oldbookdown::embed_png("screenshots/git-history-window.png", dpi = 220)
    ```
...
1.  __Push__ your changes to GitHub by clicking 
    `r oldbookdown::embed_png("screenshots/git-push.png", dpi = 220)`. (This is 
...
Once you've connected your repo to GitHub, the Git pane will show you how many commits you have locally that are not on GitHub: `r oldbookdown::embed_png("screenshots/git-local-commits.png", dpi = 220)`. This message indicates that I have 1 commit locally (my branch) that is not on GitHub ("origin/master").
...
1.  You can comment on commits. To comment on the commit as a whole, use the 
    ...
    `r oldbookdown::embed_png("screenshots/github-comment-line.png", dpi = 220)`. 
...
* In the Git pane, you see a status like
  `r oldbookdown::embed_png("screenshots/git-commit-conflict.png", dpi = 220)`
...
To resolve a merge conflict, you need to open every file with the status `r oldbookdown::embed_png("screenshots/git-commit-conflict.png", dpi = 220)`. In each file, you'll find a conflict marker that looks like this:
...
```{r, echo = FALSE}
oldbookdown::embed_png("screenshots/git-branch.png", dpi = 220)
```

If you've forgotten the name of your branch in the shell, you can use `git branch` to list all existing branches. 

If you try to synchronise this branch to GitHub from inside RStudio, you'll notice that push and pull are disabled: `r oldbookdown::embed_png("screenshots/git-no-remote.png", dpi = 220)`. To enable them, you'll need to first tell Git that your local branch has a remote equivalent:
...
1.  Switching branches: 
    ...
    ```{r, echo = FALSE}
    oldbookdown::embed_png("screenshots/github-branches.png", dpi = 220)
    ```

1.  Clicking `r oldbookdown::embed_png("screenshots/pr-create.png", dpi = 220)`

This will create a page that looks like this:

```{r, echo = FALSE}
oldbookdown::embed_png("screenshots/pr.png", dpi = 220)
```
...
* A __conversation__, 
  `r oldbookdown::embed_png("screenshots/pr-conversation.png", dpi = 220)`,
  where you can discuss the changes as a whole.
  
* The __commits__ view, 
  `r oldbookdown::embed_png("screenshots/pr-commits.png", dpi = 220)`,
  where you can see each individual commit.
  
* The __file changes__,
  `r oldbookdown::embed_png("screenshots/pr-changes.png", dpi = 220)`,
...
1.  __Fork__ the original repo by going to the repo on GitHub and clicking
    `r oldbookdown::embed_png("screenshots/github-fork.png", dpi = 220)`. 
...
```{r, echo = FALSE}
oldbookdown::embed_png("diagrams/pull-request-process.png", dpi = 220)    
```
