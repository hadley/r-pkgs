...
You might think that because no one else will ever look at your repo, that writing good commit messages is not worth the effort. However, you need to keep in mind that you have one very important collaborator: future-you! If you spend a little time now polishing your commit messages, future-you will thank you if and when future-you needs to do a post-mortem on a bug. 
...
Often, there are files that you don't want to include in the repository. They might be transient (like LaTex or C build artefacts), very large, or generated on demand. Rather that carefully _not_ staging them each time, you should instead add them to `.gitignore`. This will prevent them from ever being added. The easiest way to do this is to right-click on the file in the Git pane and select `Ignore`:
...
So far we've only been working locally, using commits to track the progress of a project and to provide safe checkpoints. However, Git really shines when you start publishing your code to the public at large. We're going to use [GitHub](http://github.com) to do this. While there are other choices, I recommend GitHub because it is free for open source projects, it has all the features you'll need and it is popular choice in the R world. 
...
To ensure your code is clean, I recommend always running `R CMD check` before you push (a topic you'll learn about in chapter on [automated checking](#check)). However, if you want to publish code that doesn't work (yet), I recommend using a branch, as you'll learn about below in [branching](#git-branch).
...
1.  It makes it easy for anyone to install your package (and benefit from your 
    hard work):
...
1.  It's easy to see the history of a file. If you navigate to a file and click 
    __History__, you'll see every commit that affected this current file. 
    Another useful view is __Blame__; it shows the last change made to each line 
    of code, who made the change, and the commit the change belongs to. This is 
    tremendously helpful when you're tracking down a bug. You can jump directly 
    to these pages from RStudio with the Git dropdown in the main toolbar: 
...
When you pull, Git first downloads (__fetches__) all of the changes and then __merges__ them with the changes that you've made. A merge is a commit with two parents. It takes two different lines of development and combines them into a single result. In many cases, Git can do this automatically: for example, if changes are made to different files, or even to different parts of the same file. However, if changes are made to the same place in a file (this is called a __merge conflict__), you'll need to resolve the conflict yourself.
...
* In the middle, the code from the last commit before the split between the
  two lines of development occurred. (This is missing in the default conflict 
...
* Merging conflicts in the `NAMESPACE` file will prevent you from re-loading or 
...
It's useful to create your own new branches when you want to (temporarily) break away from the main stream of development. Create a new branch with `git checkout -b <branch-name>`. Names should be in lower case letters and numbers, with `-` used to separate words. 
...
If you've been working one a branch for a while, other work might have been going on in the master branch. To integrate that work into your branch, run `git merge master`. You will need to resolve any merge conflicts (see above). It's best to do this fairly frequently - the less your branch has diverged from the master, the easier it will be to merge.

Once you're done working in a branch, merge it back into the master, then delete the branch:
...
(Git won't let you delete a branch unless you've merged it back into master. If you do want to abandon a branch without merging it, you'll need to force delete with `-D` instead of `-d`. If you accidentally delete a branch, don't panic. It's usually possible to get it back. See the advice about undoing mistakes).
...
Here, you'll learn about them in the context of your own code. This may seem a bit pointless because you don't _need_ them, as you can directly modify the code. But pull requests are surprisingly useful because they allow you to get feedback on a proposed changes. We use them frequently at RStudio to get feedback before merging major changes.
...
Git and GitHub are a rich and powerful set of tools, and there's no way this chapter has taught you everything you need to know. However, you should now have the basic knowledge to be effective, and you should be in a good place to learn more. Some good resources are:
...
Finally, StackOverflow is a vital part of Git - when you have a problem that you don't know how to solve, StackOverflow should be your first resource. It's highly likely that some one has had exactly the same problem as you, and there will be a variety of approaches and solutions to choose from.
