...
We recommend that each file in this directory be an `.rda` file created by `save()` containing a single object, with the same name as the file.
...
The other possibilities are described in `data()` and in the [Data in packages](https://rstudio.github.io/r-manuals/r-exts/Creating-R-packages.html#data-in-packages) section of Writing R Extensions.
...
Also notice that above, the `flights` dataset was immediately available once the package was attached with `library(nycflights13)`.
In particular, there is no need to do `data(flights)` before using `flights`.
This is another nice consequence of including `LazyData: true` in `DESCRIPTION`.
