...
-   If you want to store data persistently across R sessions, such as configuration or user-specific data, there are officially sanctioned locations for this.
...
Unlike `data/`, where we advocate for one `.rda` file per exported data object, you store all of your internal data objects together in the single file `R/sysdata.rda`.
...
*Stuff pulled from above that I'm not sure we need to re-incorporate.*
