...
This environment must be created a build time, but you can populate it with values after the package has been loaded and update those values over the course of an R session.
...
This lets you to refer to the objects inside in a very natural way, such as `the$token`, meaning "_the_ token".
...
In particular, you can't just write into the user's home directory.
...
In R, there are two tools you can use to derive acceptable locations for user data:

-   The `tools::R_user_dir()` function, available in R \>= 4.0.
-   The [rappdirs package](https://rappdirs.r-lib.org), which is a port of the Python appdirs module, compatible with R \>= 3.5[^data-3].

[^data-3]: Note that this is a sliding minimum, determined by the [tidyverse policy regarding R version support](https://www.tidyverse.org/blog/2019/04/r-version-support/).

These two tools produce filepaths consistent with the XDG spec, but implement different opinions beyond what the spec dictates.
This is easiest to see by comparing actual return values.
Note that these results were generated on macOS.

```{r eval = FALSE, include = FALSE}
# I ran this locally on macOS and inlined static results

library(tidyverse)
# to execute this chunk you also need:
# rappdirs
# datapasta
...
dat <- tibble(
  code = c(
    'rappdirs::user_data_dir("{appname}")',
    'tools::R_user_dir("{appname}", which = "data")',
    'rappdirs::user_config_dir("{appname}")',
    'tools::R_user_dir("{appname}", which = "config")',
    'rappdirs::user_cache_dir("{appname}")',
    'tools::R_user_dir("{appname}", which = "cache")'
  )
) |> 
  rowwise() |> 
  mutate(code = str_glue(code)) |> 
  mutate(result = eval(parse(text = code))) |> 
  mutate(result = ifelse(
    str_detect(result, "^~", negate = TRUE),
    str_glue("~/", fs::path_rel(result, start = "~/")),
    result
  ))

# produced the tribble call below
# datapasta::tribble_paste(dat)
# this seemed to surround data with " even though there are embedded ", so
# I hand-edited the result
```

```{r include = FALSE}
dat <- tibble::tribble(
                                         ~code,                                               ~result,
              'rappdirs::user_data_dir("pkg")',                   '~/Library/Application Support/pkg',
    'tools::R_user_dir("pkg", which = "data")', '~/Library/Application Support/org.R-project.R/R/pkg',
            'rappdirs::user_config_dir("pkg")',                   '~/Library/Application Support/pkg',
  'tools::R_user_dir("pkg", which = "config")',         '~/Library/Preferences/org.R-project.R/R/pkg',
             'rappdirs::user_cache_dir("pkg")',                                '~/Library/Caches/pkg',
   'tools::R_user_dir("pkg", which = "cache")',              '~/Library/Caches/org.R-project.R/R/pkg'
)
```

```{r echo = FALSE}
#| label: tbl-appdirs
#| tbl-cap: "Acceptable locations for persistent user data"
#| tbl-colwidths: [10,90]
...
If your package needs to write files where the filepath is not explicitly specified by the user, use one of the functions above to generate the base for the path.
