...
These datasets can be hand-picked or hand-crafted to provide compelling use cases for the functions in the package.
...
The practical details differ depending on who needs access to the data and what they will do with it:

-   If you want to store binary data (think: an R object) and make it available to the user, put it in `data/`.
...
-   If you want to store data in some raw form and make it available to the user, put it in `inst/extdata`.
...
-   If you want to store static binary data, but not make it directly available to the user, put it in `R/sysdata.rda`.
...
-   If you want to store data that reflects your package's internal state, use an environment.
...
This code is workflow code and should **not** appear in the `R/` directory of the source package.
...
In terms of advice to package users, note that lazily-loaded datasets do **not** need to be pre-loaded.
Above, once we did `library(nycflights13)`, we could immediately access `flights`.
There is no call `data(flights)`, because it is not necessary.

There are specific downsides to `data(something)` calls that support a policy of only using `data()` when it is actually necessary, i.e. for datasets that would not be available otherwise:

-   By default `data()`, creates objects in the user's global workspace. There is the potential to silently overwrite pre-existing objects with new values.
...
usethis has a convenience function that can be called when you first adopt the `data-raw/` practice or when you want to add an additional `.R` file to the folder:
...
`use_data_raw()` creates the necessary folder and file and makes the modification to `.Rbuildignore`.
...
But of course, there are plenty of legitimate reasons why you need to use non-ASCII characters in package data.
...
If you seem to have non-ASCII strings in your package data and this is unintentional, `tools::checkRdaFiles()` can be helpful for determining which `.rda` file is to blame.
...
PLUNKED HERE FOR NOW A simple alternative to these three options is to include it in the source of your package, either creating by hand, or using `dput()` to serialise an existing data set into R code.
