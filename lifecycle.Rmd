...
-   Changing the text of a print method or error.
    Caveat: This can be breaking if other packages depend on yours in regrettable ways, such as having a test that fails if you tweak an error message in your package.
...
Therefore you should take every opportunity you get to collect actual data on the consequences of a change in your package.
The tidyverse team has a fairly extensive set of tools for running so-called reverse dependency checks (@release-deps), where we run `R CMD check` on all the packages that depend on ours.

Sometimes we use this to study the impact of a potential change, i.e. we use reverse dependency checks to guide development, not only as a last-minute, pre-release check.
...
-   Experimental.
    ...
    This is the implied stage for any package with a major version of `0`, i.e. that hasn't has a `1.0.0` release yet.
...
Here's what `warn_for_verbose()` looks like (slightly simplified for this example):
...
`warn_for_verbose()` does nothing if the user leaves `verbose` unspecified or if they request `verbose = TRUE` (default behaviour).
But if the user explicitly asks for `verbose = FALSE`, we throw a warning with advice on the preferred way to suppress googledrive's messaging.
We also go ahead and honour their wishes for the time being, via the call to `local_drive_quiet()`.
...
Here are examples of how that might look:
...
But sometimes the nature of your usage makes thing trickier.
...
If `their_new_function()` doesn't exist in the latest release of otherpkg, you'll get a note from `R CMD check` stating that `their_new_function()` doesn't exist in otherpkg's namespace.
...
talk about preserving legacy behaviour to make it easier for users to apply a quick bandaid
