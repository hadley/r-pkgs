...
In ??,
we explore how to determine whether something is a breaking change or, more realistically, to gauge where it lies on a spectrum of "breakingness".
Even though it can be painful, sometimes a breaking change is beneficial for the long-term health of a package.

Since change is inevitable, the kindest thing you can do for your users to communicate clearly and help them adapt to change.
Several practices work together to achieve this:

-   Package version number: be intentional about what sort of changes are appropriate in, e.g., a patch release versus a major release (@sec-lifecycle-evolution, @sec-lifecycle-version-number).

-   Lifecycle stage: be explicit when a function or argument is regarded as experimental, superseded, or deprecated, as opposed to stable (the assumed default).

-   Deprecation process: enact change in a phased way, which makes it easier for users to adjust their code.

The main form of user-facing change is a package release () and one of the most overt signals about the nature of change is the version number ().
Having established the meaning of a breaking change and the mechanics of version numbers, we can lay out our conventions for different types of releases in ??.
There we describe what sort of change is considered appropriate for a major versus minor versus patch release.

The last piece of lifecycle infrastructure we'll cover is the concept of lifecycle stages: experimental, stable, superseded, and deprecated.
These stages can apply to a function or an argument or even a whole package.
The tidyverse team maintains various helpful utilities in the lifecycle package, such as badges that can be used in documentation and specialized functions for communicating lifecycle changes to users.
We can then discuss how specific lifecycle transitions connect back to our conventions about different types of releases.

(TODO: I suspect this will split off into its own chapter, again.) Finally, in ??
...
In section @sec-lifecycle-version-number, we'll delve into the world of software version numbers, which is a richer topic than you might expect.
...
Here we're going to talk about how to assess whether a change is breaking and in section @sec-lifecycle-backward-compatibility we'll talk about how to decide if a breaking change is worth it.
...
This difference is a bit academic in the R community, because of the way most users manage package installation.
...
We suspect that package-specific libraries and tools like renv are currently under-utilized in the R world.
...
In section ??,
we describe how tidyverse packages approach this, supported by tools in the lifecycle package.
...
## Lifecycle stages and tools and package

*obviously the name of this section need to change*

The tidyverse team's approach to package evolution has become more structured and deliberate over the years.
The system relies on two major components:
...
-   Deprecation tools and conventions, which we use when transitioning a function from one lifecycle stage to another.

The lifecycle package is the primary home for these tools and its documentation is a great place to learn more ([lifecycle.r-lib.org](https://lifecycle.r-lib.org/index.html)).
We won't duplicate too much of those details here.
Instead, we highlight the general principles of lifecycle management and present some specific examples of successful lifecycle "moves".
...
The lifecycle stage is most often communicated through a badge.
...
This leaves you in a position to use lifecycle badges in help topics (and to use lifecycle functions, which we discuss in ??).
...
Here's how the deprecation of `readr::write_file(path =)` is indicated:
...
Call `usethis::use_lifecycle_badge()` if you want to use a badge in README to indicate the lifecycle of an entire package.

If the lifecycle of a package is "stable", we might not bother with a badge, as "stable" is the assumed default.
Similarly, we typically only use a badge for a function or argument if its stage differs from the overall stage of the package.

### Deprecating a function or argument

If you're going to remove a function or argument, it's usually best to do so in phases.
...
When you're deprecating an argument, you can also use `lifecycle::deprecate()` as its default value.
...
TODO: Say more about `deprecated()`.
What's the point?
Visual signal to the user + works with `lifecycle::is_present()`.
Why we think it's OK to call it as `deprecated()` and not `lifecycle::deprecated()`.

Here's a hypothetical blueprint for removing a function `fun()`:

-   Package version `0.5.0`: `fun()` exists.
    The lifecycle stage of the package is "experimental", as indicated by the pre-`1.0.0` version number and, perhaps, a package-level badge.
    The lifecycle stage of `fun()` is also "experimental", by extension.

-   Package version `0.6.0`: The deprecation process of `fun()` begins.
    We insert `r lifecycle::badge("deprecated")` in its `@description` to place a badge in its help topic.
    In the body of `fun()`, we add a call to `lifecycle::deprecate_warn()` to inform users about the situation.

-   Package version `0.7.0` or `1.0.0`: `fun()` is removed.
...
TODO: mention `.Deprecated()` as the base way.
Also maybe mention `!missing(x)` as a base alternative to `deprecated()` + `is_present()`.

TODO: update everything after this point.
