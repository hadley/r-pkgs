...
-   Changing the text of a print method or error.
    Caveat: This can be breaking if other packages depend on yours in fragile ways, such as having a test that fails if you tweak an error message in your package.
...
The tidyverse team has a fairly extensive set of tools for running so-called reverse dependency checks (@sec-release-revdepcheck), where we run `R CMD check` on all the packages that depend on ours.
Sometimes we use this to study the impact of a potential change, i.e. reverse dependency checks can be used to guide development, not only as a last-minute, pre-release check.
...
From a marketing perspective, you probably want to save these for a major release, because your users are more likely to learn about the new goodies.

Here are a few blog posts that have accompanied different types of release for tidyverse packages:
...
In section @sec-lifecycle-stages-and-package, we describe how tidyverse packages approach this, supported by tools in the lifecycle package.
...
In this example, the `add3()` function is going to be replaced with `plus3()`:
...
-   A warning is only issued if the person reading it is the one who can actually do something about it, i.e. update the code.
...
In this case, it's also handy to use `lifecycle::deprecate()` as the default value for the deprecated argument.
...
somewhat similar to deprecating function, in that is the most useful function.
For detailed coverage, see `vignette("communicate", package = "lifecycle")`.
When you're deprecating an argument, you can also use .
