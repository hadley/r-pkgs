...
Here are some concrete examples of breaking changes:
...
Your package should start with the version number `0.0.0.9000`.
...
The associated tooling and documentation live in the lifecycle package ([lifecycle.r-lib.org](https://lifecycle.r-lib.org/index.html)).
...
Here's how the deprecation of `readr::write_file(path =)` is documented[^lifecycle-6]:

[^lifecycle-6]: Since this was originally written, the `path` argument has been completely removed from `readr::write_file()`.
